...
import { 
  ...
  PanelLeftClose, PanelLeftOpen, Sun, Moon, 
  LayoutGrid, Star, ShieldAlert, Ban, Maximize,
  ListTodo, Activity, Clock, Loader2, 
  WifiOff, SignalLow, Image as ImageIcon, Lock, Wifi,
  CheckSquare, AlertTriangle, RefreshCcw, Zap, EyeOff
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
...
const Atmosphere = ({ isDarkMode }) => (
    <div className={`fixed inset-0 overflow-hidden pointer-events-none transition-colors duration-700 z-0 ${isDarkMode ? 'bg-[#050B14]' : 'bg-slate-50'}`}>
        <motion.div 
            animate={{ scale: [1, 1.2, 1], opacity: isDarkMode ? [0.2, 0.4, 0.2] : [0.4, 0.6, 0.4] }}
            transition={{ duration: 8, repeat: Infinity }}
            className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] sm:w-[800px] sm:h-[800px] rounded-full blur-[100px] sm:blur-[120px] ${isDarkMode ? 'bg-indigo-900/40' : 'bg-blue-100'}`}
        />
    </div>
);
...
const getTheme = (isDarkMode) => ({
    ...
    optionIdle: isDarkMode ? "bg-[#1E293B]/80 border-slate-700 hover:bg-[#334155]" : "bg-white/80 border-slate-200 hover:border-blue-400 hover:shadow-md",
    optionActive: isDarkMode ? "bg-blue-600/20 border-blue-500 shadow-[0_0_0_1px_rgba(59,130,246,0.5)]" : "bg-blue-50/50 border-blue-600 shadow-sm",
    // Added specific style for locked selected options
    optionLockedSelected: isDarkMode ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-500" : "bg-emerald-50 border-emerald-300 text-emerald-700",
    navBtn: isDarkMode ? "bg-[#1E293B] hover:bg-slate-800 text-white border border-slate-700" : "bg-white hover:bg-slate-50 text-slate-700 border border-slate-200",
    primaryBtn: "bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-500/30",
});
...
const slideVariants = {
  ...
  center: { x: 0, opacity: 1, scale: 1, transition: { type: "spring", stiffness: 350, damping: 25, opacity: { duration: 0.2 } } },
  exit: (direction) => ({ x: direction < 0 ? 30 : -30, opacity: 0, scale: 0.98, transition: { type: "spring", stiffness: 350, damping: 25, opacity: { duration: 0.2 } } })
};

/* --- 3. MODALS & OVERLAYS --- */

const FocusLostOverlay = ({ onResume, isDarkMode }) => (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/60 backdrop-blur-2xl animate-in fade-in duration-300">
        <div className={`text-center max-w-md p-8 rounded-3xl border shadow-2xl ${isDarkMode ? 'bg-[#0F172A] border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
            <div className="w-20 h-20 mx-auto rounded-full bg-amber-500/20 flex items-center justify-center mb-6 animate-pulse">
                <EyeOff size={40} className="text-amber-500" />
            </div>
            <h2 className="text-2xl font-black mb-2">Session Paused</h2>
            <p className={`mb-8 text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                You have navigated away from the quiz window or exited full screen. 
                <br/><strong>This action has been logged.</strong>
            </p>
            <button 
                onClick={onResume} 
                className="w-full py-4 rounded-xl font-bold uppercase tracking-widest bg-amber-500 hover:bg-amber-600 text-white shadow-lg shadow-amber-500/30 transition-all active:scale-95"
            >
                Resume Session
            </button>
        </div>
...
const SubmitReviewModal = ({ isOpen, onClose, onConfirm, answers, quizData, isDarkMode, jumpToQuestion, isSubmitting, submitError }) => {
    ...
    if (submitError) {
        return (
            <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
                <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className={`w-full max-w-md p-6 sm:p-8 rounded-3xl border-2 border-red-500 shadow-2xl flex flex-col items-center text-center ${isDarkMode ? 'bg-[#0F172A] text-white' : 'bg-white text-slate-900'}`}>
                    ...
                    <div className="flex gap-3 w-full">
                         <button onClick={onClose} className={`flex-1 py-3 rounded-xl font-bold text-sm ${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>Close</button>
                         <button onClick={onConfirm} disabled={isSubmitting} className="flex-1 py-3 rounded-xl font-bold text-sm bg-red-600 text-white shadow-lg flex items-center justify-center gap-2">
                            {isSubmitting ? <Loader2 className="animate-spin" size={16} /> : "Retry"}
                         </button>
                    </div>
                </motion.div>
            </div>
    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={`w-full max-w-2xl p-6 rounded-3xl border shadow-2xl overflow-hidden flex flex-col max-h-[85vh] ${isDarkMode ? 'bg-[#0F172A] border-slate-700 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
                <div className="text-center mb-6">
                    <ListTodo size={40} className="mx-auto text-blue-500 mb-2" />
                    <h2 className="text-xl sm:text-2xl font-black">Summary</h2>
                </div>
                <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className={`p-3 rounded-2xl border text-center ${isDarkMode ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-emerald-100 border-emerald-200'}`}>
                        <div className={`text-2xl font-black ${isDarkMode ? 'text-emerald-500' : 'text-emerald-700'}`}>{attempted}</div>
                        <div className="text-[10px] font-bold uppercase tracking-wider opacity-70">Attempted</div>
                    </div>
                    <div className={`p-3 rounded-2xl border text-center ${isDarkMode ? 'bg-amber-500/10 border-amber-500/20' : 'bg-amber-100 border-amber-200'}`}>
                        <div className={`text-2xl font-black ${isDarkMode ? 'text-amber-500' : 'text-amber-700'}`}>{skipped}</div>
                        <div className="text-[10px] font-bold uppercase tracking-wider opacity-70">Skipped</div>
                    </div>
                </div>

                <div className="flex-1 overflow-y-auto min-h-[120px] mb-6 pr-2 custom-scrollbar">
                    <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-2">
                        {quizData.map((q, idx) => {
                             const val = answers[q._id];
                             const hasAnswer = Array.isArray(val) ? val.length > 0 : !!val;
                             let btnClass = hasAnswer 
                                ? (isDarkMode ? "bg-emerald-500/20 text-emerald-400 border-emerald-500/30" : "bg-emerald-100 text-emerald-700 border-emerald-300") 
                                : (isDarkMode ? "bg-slate-800 text-slate-500 border-slate-700" : "bg-slate-100 text-slate-400 border-slate-200");
                             return ( <button key={q._id} onClick={() => jumpToQuestion(idx)} className={`h-8 w-8 rounded-md flex items-center justify-center text-xs font-bold border ${btnClass}`}>{idx + 1}</button> );
                        })}
                    </div>
                </div>
                <div className="flex gap-3 mt-auto pt-4 border-t border-inherit">
                    <button onClick={onClose} disabled={isSubmitting} className={`flex-1 py-3 rounded-xl font-bold text-sm ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700'}`}>Review</button>
                    <button onClick={onConfirm} disabled={isSubmitting} className="flex-1 py-3 rounded-xl font-bold text-sm bg-blue-600 text-white shadow-lg flex items-center justify-center gap-2">
...
const WarningModal = ({ isOpen, onClose, warningsLeft, message }) => {
    ...
    return (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="w-full max-w-sm p-6 rounded-2xl border-2 border-red-500 bg-[#0F172A] text-white text-center shadow-2xl">
                <ShieldAlert size={48} className="text-red-500 mx-auto mb-4" />
                <h2 className="text-2xl font-black mb-2">Warning</h2>
                <p className="text-slate-300 mb-6 text-sm">{message}</p>
                {warningsLeft !== undefined && (
                     <div className="inline-block px-4 py-1.5 rounded-lg bg-red-500/20 border border-red-500/30 text-red-400 font-bold text-xs mb-6 uppercase tracking-wider">
                        {warningsLeft} Attempts Remaining
                     </div>
                )}
                <button onClick={onClose} className="w-full py-3 bg-red-600 rounded-xl font-bold text-sm uppercase tracking-wider hover:bg-red-500">I Understand</button>
            </motion.div>
        </div>
    );
};
...
const LockoutModal = ({ message }) => (
    <div className="fixed inset-0 z-[140] flex items-center justify-center p-4 bg-black/95 backdrop-blur-xl">
        <div className="text-center text-white max-w-lg">
            <Ban size={64} className="text-red-500 mx-auto mb-6" />
            <h1 className="text-3xl font-black mb-4">Terminated</h1>
            <p className="text-slate-400 mb-8">{message || "Security limit exceeded. Auto-submitting..."}</p>
        </div>
    </div>
);
...
const FullscreenGate = ({ onEnter }) => (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl">
        <div className="text-center text-white max-w-md">
            <Maximize size={48} className="text-indigo-400 mx-auto mb-6" />
            <h1 className="text-2xl sm:text-3xl font-black mb-4">Assessment Mode</h1>
            <p className="text-slate-400 mb-8 text-sm sm:text-base">
                Full-screen is required. 
                <br/><span className="text-red-400 text-xs mt-2 block">Keyboard input and DevTools are disabled.</span>
            </p>
            <button onClick={onEnter} className="w-full py-4 bg-indigo-600 rounded-xl font-bold uppercase tracking-widest shadow-lg shadow-indigo-500/30 hover:bg-indigo-500">Start Assessment</button>
        </div>
    </div>
);
...
const QuizActivePage = () => {
  ...
  const apiData = location.state || {};
  const { student, session, questions } = apiData; 
  const activeSessionCode = session?.sessionId || apiData.sessionCode; 

  // [SECURITY 1] ENFORCED ENTRY GUARD
  useEffect(() => {
    if (!questions || !session) {
        console.warn("Missing quiz data");
    }
  }, [questions, session, navigate]);
  ...
  const [questionStartTime, setQuestionStartTime] = useState(Date.now());
  const [timeLeft, setTimeLeft] = useState(session?.durationMinutes ? session.durationMinutes * 60 : 2700); 
  ...
  /* --- SECURITY: KEYBOARD BLOCKER & DEVTOOLS DISABLE --- */
  useEffect(() => {
    const blockInput = (e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    window.addEventListener('keypress', blockInput, { capture: true });
    window.addEventListener('contextmenu', disableContextMenu);
    return () => {
        window.removeEventListener('keydown', blockInput, { capture: true });
        window.removeEventListener('keyup', blockInput, { capture: true });
        window.removeEventListener('keypress', blockInput, { capture: true });
  ...
  const handleViolation = useCallback(async (type) => {
      ...
      try {
        ...
        if (data.action === "WARN") {
          ...
          console.error("Violation logging error:", error);
          setServerMessage("Security alert. Please return to the quiz.");
          setShowWarning(true);
      }
  }, [isLocked, hasStarted, isSubmitting, activeSessionCode]); 

  /* --- SECURITY: FOCUS/BLUR/FULLSCREEN DETECTORS --- */
  useEffect(() => {
      if (!hasStarted) return;
  ...
  const resumeSession = () => {
      ...
      if (elem.requestFullscreen) {
          elem.requestFullscreen().then(() => setIsFocusLost(false)).catch(() => setIsFocusLost(false));
      } else {
  ...
  const handleSelect = (optionId) => {
    ...
    setAnswers(prev => {
        const prevAnswer = prev[currentQ._id];
        let newAnswer;

        if (currentQ.type === 'checkbox') {
            const currentList = Array.isArray(prevAnswer) ? prevAnswer : [];
            if (currentList.includes(optionId)) {
                newAnswer = currentList.filter(id => id !== optionId);
            } else {
                newAnswer = [...currentList, optionId];
            }
        } else {
            newAnswer = optionId;
        }
        return { ...prev, [currentQ._id]: newAnswer };
    });
  };

  // --- CORE LOGIC: LOCK & NAVIGATE ---
  const navigateQuestion = (newIndex) => {
      if (newIndex === currentQIndex) return;
      
      const currentAnswer = answers[currentQ._id];
      const hasAns = Array.isArray(currentAnswer) ? currentAnswer.length > 0 : !!currentAnswer;
      
      // IF ANSWERED AND NOT ALREADY LOCKED, LOCK IT AND SUBMIT
      if (hasAns && !lockedQuestions.has(currentQ._id)) {
          const timeTaken = Math.max(1, Math.floor((Date.now() - questionStartTime) / 1000));
          ...
          // 2. Lock locally
          setLockedQuestions(prev => new Set(prev).add(currentQ._id));
      }
  ...
  // --- CORE LOGIC: SUBMIT BUTTON (LAST QUESTION) ---
  const handleFinalSubmitClick = () => {
      // Logic to lock the current (last) question if answered before showing modal
      const currentAnswer = answers[currentQ._id];
      const hasAns = Array.isArray(currentAnswer) ? currentAnswer.length > 0 : !!currentAnswer;
  ...
  const finishQuiz = useCallback(async () => {
      ...
      try {
          const response = await fetch(`${BACKEND_URL}/api/student/quiz/finish`, {
              ...
              headers: { 'Content-Type': 'application/json' },
              credentials: 'include',
              body: JSON.stringify({ sessionCode: activeSessionCode, finalAnswers: answers }) 
          });
          const result = await response.json();
          if (result.success) {
              if (document.fullscreenElement) document.exitFullscreen().catch(() => {});
              navigate('/student/quiz/result', { state: { ...result.data, theme: isDarkMode ? 'dark' : 'light' }, replace: true });
          } else {
              setSubmitError(result.message || "Submission failed.");
              setIsSubmitting(false);
          }
  ...
  }, [activeSessionCode, navigate, isSubmitting, isDarkMode, answers]);
  ...
  if (!questions || !session) return <div className="p-10 text-center">Loading Assessment...</div>;

  const formatTime = (s) => `${Math.floor(s / 60)}:${(s % 60).toString().padStart(2, '0')}`;
  
  const getTimerStyles = () => {
      if (timeLeft < 60) return "bg-red-500 text-white border-red-600 animate-pulse shadow-red-500/50";
      if (timeLeft < 300) return "bg-yellow-400 text-black border-yellow-500 shadow-yellow-400/50"; 
      return isDarkMode ? 'bg-[#1E293B] text-blue-400 border-slate-700' : 'bg-white text-slate-700 border-slate-200 shadow-sm';
  };

  const hasLongText = currentQ.options.some(opt => opt.text.length > 60);
  const useGridLayout = !hasLongText && currentQ.options.length > 1;
  const isQuestionLocked = lockedQuestions.has(currentQ._id); // Check current Q status

  /* --- RENDER --- */
  const NetworkIndicator = () => {
      const isWeak = connectionStatus.effectiveType === '2g' || connectionStatus.effectiveType === 'slow-2g';
      return (
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all duration-300 text-xs font-bold uppercase
              ${!connectionStatus.online ? 'bg-red-500/10 text-red-500 border-red-500/20' : isDarkMode 
                  ? (isWeak ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500')
                  : (isWeak ? 'bg-amber-100 border-amber-200 text-amber-700' : 'bg-emerald-100 border-emerald-200 text-emerald-700')
              }`}
          >
              {!connectionStatus.online ? <WifiOff size={14} /> : isWeak ? <SignalLow size={14} /> : <Wifi size={14} />}
              <span className="hidden sm:inline">
                  {!connectionStatus.online ? 'Offline' : isWeak ? 'Weak' : 'Online'}
              </span>
          </div>
      );
  };
  ...
  const QuizAtlas = () => (
        <div className={`flex flex-col h-full border-r transition-colors duration-300 ${isDarkMode ? 'bg-[#0F172A] border-slate-800' : 'bg-white border-slate-200'}`}>
          <div className="p-6 flex flex-col items-center text-center gap-3 border-b border-inherit">
              <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-blue-500/50 bg-slate-200 shadow-inner flex-shrink-0">
                  <img 
                    src={`${ASSET_BASE_URL}/STUDENTS/${student?.rollno}/${student?.rollno}.jpg`}
                    onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${student?.name}&background=random`; }}
                    alt="Profile" 
                    className="w-full h-full object-cover"
                  />
              ...
              <div className="space-y-0.5">
                  <h3 className={`font-bold text-xs leading-tight uppercase ${t.text}`}>{student?.name}</h3>
                  <p className={`text-[10px] font-bold tracking-widest opacity-50 uppercase ${t.text}`}>{student?.rollno}</p>
              </div>
          </div>
          <div className="p-4 pb-2 border-b border-inherit flex items-center gap-2">
             <LayoutGrid size={14} className="text-indigo-400" />
             <span className={`text-[10px] font-black uppercase tracking-widest ${t.textSecondary}`}>Map</span>
          </div>
          <div className="flex-1 overflow-y-auto px-4 py-4 scrollbar-hide">
            <div className="grid grid-cols-5 gap-2">
              {questions.map((q, idx) => {
                 ...
                 // Visual lock indicator on map if we want? For now, standard coloring.
                 let btnClass = isActive 
                    ? "bg-blue-600 text-white shadow-md ring-1 ring-blue-400 scale-105"
                    : isAnswered ? (isDarkMode ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-emerald-100 text-emerald-700 border border-emerald-200")
                    : (isDarkMode ? "bg-[#1E293B] text-slate-500 border border-slate-800" : "bg-white text-slate-400 border border-slate-200");
                 return ( <button key={q._id} onClick={() => navigateQuestion(idx)} className={`h-8 w-8 rounded-lg flex items-center justify-center text-[10px] font-bold transition-all relative ${btnClass}`}>{idx + 1}</button> )
              })}
            </div>
          </div>
          <div className={`p-4 mt-auto border-t ${t.border}`}>
              <button onClick={() => setIsReviewing(true)} className={`w-full py-3 rounded-xl font-bold text-xs shadow-lg ${t.primaryBtn}`}>Finish</button>
          </div>
        </div>
    );
  ...
  return (
    <div className={`h-[100dvh] flex flex-col font-sans overflow-hidden transition-colors duration-500 ${t.text} select-none`}>
      ...
      {!hasStarted && <FullscreenGate onEnter={enterFullscreen} />}
      {isLocked && <LockoutModal message={serverMessage} />}
      {isFocusLost && hasStarted && !isLocked && !isSubmitting && <FocusLostOverlay onResume={resumeSession} isDarkMode={isDarkMode} />}
      
      <WarningModal isOpen={showWarning} onClose={() => { setShowWarning(false); resumeSession(); }} warningsLeft={warningsLeft} message={serverMessage} />
      ...
      <header className={`h-16 sm:h-20 border-b flex items-center justify-between px-4 sm:px-6 z-20 relative backdrop-blur-md flex-shrink-0 ${isDarkMode ? 'bg-[#0F172A]/80 border-slate-800' : 'bg-white/80 border-slate-200'}`}>
        <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden lg:flex p-2 rounded-xl text-slate-400 transition-colors hover:bg-slate-700/20">{sidebarOpen ? <PanelLeftClose size={20} /> : <PanelLeftOpen size={20} />}</button>
            <button className="lg:hidden p-2 rounded-xl text-slate-400" onClick={() => setMobileMenuOpen(true)}><Menu size={20} /></button>
            <div className="hidden sm:block ml-2">
                <h1 className="text-sm sm:text-base font-black truncate max-w-[150px] sm:max-w-xs">{session?.title}</h1>
            </div>
        </div>

        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className={`flex items-center gap-2 sm:gap-3 px-4 py-1.5 rounded-full font-mono font-bold border transition-all duration-300 ${getTimerStyles()}`}>
                {timeLeft < 300 ? <Activity size={16} className={timeLeft < 60 ? "animate-bounce" : ""} /> : <Clock size={16} />}
                <span className="text-base sm:text-lg tracking-widest">{formatTime(timeLeft)}</span>
            </div>
        </div>

        <div className="flex items-center gap-3">
             <NetworkIndicator />
            <div onClick={() => setIsDarkMode(!isDarkMode)} className="relative h-9 w-16 sm:h-10 sm:w-20 rounded-full cursor-pointer p-1 bg-slate-800 border border-slate-700">
                <div className="flex justify-between items-center w-full h-full px-1.5"><Moon size={14} className={isDarkMode ? "text-white" : "text-slate-400"} /><Sun size={14} className={!isDarkMode ? "text-orange-500" : "text-slate-600"} /></div>
                <motion.div layout className={`absolute top-1 bottom-1 w-[28px] sm:w-[34px] rounded-full shadow-sm ${isDarkMode ? 'left-1 bg-slate-600' : 'right-1 bg-white'}`} />
            </div>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden relative z-10">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 lg:p-10 pb-24 scroll-smooth custom-scrollbar">
            <AnimatePresence mode="wait" custom={direction}>
                <motion.div 
                    key={currentQ._id} 
                    custom={direction} 
                ...
                >
                        ...
                        <div className="flex justify-between items-center mb-4 sm:mb-6">
                            <span className={`text-[10px] sm:text-xs font-black px-3 py-1 sm:px-4 sm:py-1.5 rounded-full uppercase border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>Q {currentQIndex + 1}</span>
                            <div className="flex gap-2">
                                {/* LOCKED BADGE */}
                                {isQuestionLocked && (
                                    <span className={`text-[10px] sm:text-xs font-bold px-3 py-1 sm:px-3 sm:py-1.5 rounded-full uppercase border flex items-center gap-1 ${isDarkMode ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-rose-100 border-rose-200 text-rose-700'}`}>
                                        <Lock size={10} /> Locked
                                    </span>
                                )}
                                <span className={`text-[10px] sm:text-xs font-bold px-3 py-1 sm:px-3 sm:py-1.5 rounded-full uppercase border flex items-center gap-1 ${isDarkMode ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-amber-100 border-amber-200 text-amber-700'}`}>
                                    <Star size={10} fill="currentColor"/> {currentQ.marks}
                                </span>
                                {currentQ.type === 'checkbox' && (
                                     <span className={`text-[10px] sm:text-xs font-bold px-3 py-1 sm:px-3 sm:py-1.5 rounded-full uppercase border flex items-center gap-1 ${isDarkMode ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400' : 'bg-indigo-100 border-indigo-200 text-indigo-700'}`}>
                        ...
                        {/* QUESTION TEXT BOX */}
                        <div className={`p-6 sm:p-8 md:p-10 rounded-[1.5rem] sm:rounded-[2rem] mb-6 sm:mb-8 flex flex-col gap-4 sm:gap-6 ${t.card}`}>
                            <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-relaxed">{currentQ.questionText}</h2>
                            {currentQ.image && (
                                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full relative rounded-2xl overflow-hidden border border-inherit bg-black/5">
                                    <img src={currentQ.image} alt="Visual" className="w-full h-auto max-h-[250px] sm:max-h-[350px] object-contain mx-auto" />
                                    <div className="absolute top-2 right-2 bg-black/40 backdrop-blur-md p-1.5 rounded-lg text-white"><ImageIcon size={14} /></div>
                                </motion.div>
                            )}
                        </div>

                        {/* OPTIONS */}
                        <div className={useGridLayout ? "grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 mb-auto" : "space-y-3 sm:space-y-4 mb-auto"}>
                        {currentQ.options.map((opt) => {
                            const val = answers[currentQ._id];
                            const isCheckbox = currentQ.type === 'checkbox';
                            const isSelected = isCheckbox ? (Array.isArray(val) && val.includes(opt._id)) : (val === opt._id);
                            
                            // VISUAL LOGIC FOR LOCKED/UNLOCKED
                            let optionStyle = t.optionIdle;
                            if (isSelected) {
                                if (isQuestionLocked) optionStyle = t.optionLockedSelected; // New Locked Style
                                else optionStyle = t.optionActive;
                            }
                            ...
                            return (
                                <div 
                                    key={opt._id} 
                                    onClick={() => handleSelect(opt._id)} 
                                    className={`group flex items-center gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl border-2 transition-all select-none
                                        ${optionStyle}
                                        ${isQuestionLocked ? 'cursor-not-allowed opacity-75' : 'cursor-pointer'}
                                    `}
                                >
                                    <div className={`w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0 flex items-center justify-center rounded-${isCheckbox ? 'md' : 'full'} border-2 
                                        ${isSelected ? (isQuestionLocked ? 'border-emerald-500 bg-emerald-500' : 'border-blue-600 bg-blue-600') : 'border-slate-600'}
                                    `}>
                                        {isSelected && (
                                            isCheckbox ? <CheckSquare size={12} className="text-white" /> : <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 bg-white rounded-full" />
                                        )}
                                    </div>
                                    <span className={`text-sm sm:text-base font-medium ${isSelected ? (isQuestionLocked ? 'text-emerald-600' : t.text) : t.textSecondary}`}>{opt.text}</span>
                                    {isQuestionLocked && isSelected && <Lock size={16} className="ml-auto text-emerald-500" />}
                                </div>
                            )
                        })}
                        </div>
                        ...
                        {/* NAVIGATION BUTTONS */}
                        <div className="flex justify-between pt-6 sm:pt-8 mt-auto pb-4">
                            <button 
                                onClick={() => navigateQuestion(Math.max(0, currentQIndex - 1))} 
                                disabled={currentQIndex === 0} 
                                className={`flex items-center gap-2 px-5 py-2.5 sm:px-6 sm:py-3 rounded-xl font-bold text-sm sm:text-base ${t.navBtn}`}
                            >
                            ...
                            {currentQIndex === questions.length - 1 ? ( 
                                <button 
                                    ...
                                    className={`px-6 py-2.5 sm:px-8 sm:py-3 rounded-xl font-bold text-sm sm:text-base ${t.primaryBtn}`}
                                >
                                    Submit
                                </button> 
                            ) : ( 
                                <button 
                                    onClick={() => navigateQuestion(Math.min(questions.length - 1, currentQIndex + 1))} 
                                    className={`flex items-center gap-2 px-6 py-2.5 sm:px-8 sm:py-3 rounded-xl font-bold text-sm sm:text-base ${t.primaryBtn}`}
                                >
        ...
        <AnimatePresence>
            {mobileMenuOpen && (
                <motion.div className="absolute inset-0 z-50 lg:hidden">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)} />
                    <motion.div initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} className="absolute left-0 top-0 bottom-0 w-[260px] bg-[#0F172A]"><QuizAtlas /></motion.div>
                </motion.div>
            )}
        </AnimatePresence>
      </div>
