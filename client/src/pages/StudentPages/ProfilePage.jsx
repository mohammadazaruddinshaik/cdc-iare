...
//     BadgeCheck,
//     Trophy, Medal, Star,
//     Edit2, Save, Link as LinkIcon, ExternalLink
// } from 'lucide-react';
...
// const ProfileDetailCard = ({ icon, label, value, description, variant = 'default', isImage = false, link = null }) => {
//     const variants = {
...
//                             className: `w-6 h-6 ${variant === 'primary' ? 'text-blue-600' : variant === 'secondary' ? 'text-purple-600' : variant === 'gold' ? 'text-amber-600' : 'text-gray-700'}`
//                         })
//                     )}
//                 </div>
...
//                             {link && <LinkIcon size={12} className="text-blue-500" />}
//                             <p className={`text-xs font-medium px-2 py-0.5 rounded-md truncate max-w-full ${link ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'}`}>
//                                 {description}
//                             </p>
//                         </div>
//                     )}
...
// // Updated UpdateHandlesModal with Modern B&W UI
...
//                                 <span className="bg-gray-100 text-gray-600 text-xs px-2.5 py-1 rounded-full font-bold border border-gray-200">
//                                     Live Data
//                                 </span>
//                             </h2>
//                             {/* Updated Edit Handles Button */}
//                             <button 
...
//                                 link={userData.codingProfile.handles.leetcode}
//                             />
//                             <ProfileDetailCard 
//                                 icon={gfgImg}
//                                 isImage={true}
...
//                                 link={userData.codingProfile.handles.gfg}
//                             />
//                             <ProfileDetailCard 
//                                 icon={ccImg}
//                                 isImage={true}
...
//                                 link={userData.codingProfile.handles.codechef}
//                             />
//                             <ProfileDetailCard 
//                                 icon={ghImg}
//                                 isImage={true}
...
//                                 link={userData.codingProfile.handles.github}
//                             />
//                         </div>
//                     </section>
...
const backendUrl = import.meta.env.VITE_BASE_URL;
// Ensure this key matches your backend key exactly
const EncDec_SECRET_KEY = import.meta.env.VITE_ENC_SECRET_KEY || "YOUR_FALLBACK_KEY"; 
...
const encryptData = (data) => {
    try {
        if (!data) return null;
        // Convert object to string, or keep string as is
...
const UpdateHandlesModal = ({ isOpen, onClose, currentHandles, onUpdateSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 3. DECRYPT RESPONSE (if 'data' key exists, otherwise try rawResult)
            // If backend returns unencrypted error on 4xx/5xx, logic below handles it gracefully.
            const result = rawResult.data ? decryptData(rawResult.data) : rawResult;

            if (!response.ok) {
                // Use decrypted result message
                setApiMessage({ type: 'error', text: result?.message || 'Failed to update handles' });
                setIsLoading(false);
                return;
            }
            ...
            if (result && result.codingProfile) {
                setTimeout(() => {
                    onUpdateSuccess(result.codingProfile);
                    onClose();
                }, 1000);
...
const ProfilePage = () => {
    ...
    useEffect(() => {
        ...
        const fetchProfileData = async () => {
            try {
                // GET requests: No Body to Encrypt, but Response needs Decryption
                const response = await fetch(`${backendUrl}/api/student/get-profile-data?rollno=${user.username}`, {
                    method: 'GET',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: "include",
                ...
                // 2. DECRYPT RESPONSE (Check if 'data' key exists)
                const data = rawData.data ? decryptData(rawData.data) : (rawData.student ? rawData : decryptData(rawData)); 

                // Fallback check to ensure we have data
                if (!data) throw new Error("Failed to decrypt data");
