...
import { 
    ChevronLeft, Timer, ChevronDown, RotateCcw, Play, Loader2, Rocket,
    Check, X, Edit3, Maximize,
    Plus, Terminal, Cpu, Zap, CornerDownRight, Box, 
    LayoutTemplate, Columns, Minus, AlertOctagon, 
    Award, Info, Hash, TriangleAlert, Ban
} from 'lucide-react';
...
const THEMES = {
    light: {
        ...
        textMain: "text-slate-600",
        textHead: "text-slate-800",
        textSec: "text-slate-400",
        border: "border-slate-200", 
        inputBg: "bg-white", 
        inputBoxHighContrast: "bg-white text-slate-800 border-slate-300 placeholder:text-slate-400 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/20",
        codeBlock: "bg-slate-100 border-slate-200 text-slate-700", 
        codeHeader: "bg-slate-50 border-b border-slate-200",
        accentPrimary: "bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-slate-200",
        monaco: "light",
        scrollTrack: "bg-slate-100",
        modalOverlay: "bg-slate-900/20",
        successBadge: "bg-emerald-50 text-emerald-600 border-emerald-100",
        errorBadge: "bg-rose-50 text-rose-600 border-rose-100",
        selectedLayout: "ring-2 ring-slate-900 bg-slate-50"
    },
    midnight: { 
        id: 'midnight', 
...
const getDifficultyColor = (difficulty, themeId) => {
    const diff = (difficulty || 'Medium').toLowerCase();
    if (themeId === 'light') {
        if (diff === 'easy') return "bg-emerald-50 text-emerald-700 border-emerald-100";
        if (diff === 'medium') return "bg-amber-50 text-amber-700 border-amber-100";
...
const ProblemSolverPage = () => {
    ...
    // Violation Logic
    const [violationData, setViolationData] = useState(null); // { action, message, warningsLeft }
    ...
    const reportCheatAttempt = async (violationType) => {
        // Prevent duplicate calls if we are already submitting due to termination
        if (isAutoSubmitting.current) return;

        console.warn(`[Anti-Cheat] Reporting: ${violationType}`);
        
        try {
            const res = await api.post('/api/student/cheat', {
                examId: contestId, 
                violationType: violationType
            });

            const { action, message, warningsLeft } = res.data;

            if (action === 'TERMINATE') {
                // 1. Show Termination Modal
                setViolationData({ 
                    action: 'TERMINATE', 
                    message: message || "Violation limit exceeded. Terminating...", 
                    warningsLeft: 0 
                });
                setShowViolationModal(true);
                
                // 2. Lock everything
                isAutoSubmitting.current = true; // Prevent other triggers
                
                // 3. Auto Submit after brief delay (so user sees the message)
                setTimeout(() => {
                ...
                setViolationData({ 
                    action: 'WARN', 
                    message: message || "Warning! Cheating detected.", 
    ...
    // 1. Listeners for Cheating Attempts
    useEffect(() => {
        const handleContextMenu = (e) => { 
            e.preventDefault(); 
            reportCheatAttempt("contextmenu");
        };
        const handleKeyDown = (e) => {
            if (e.ctrlKey || e.metaKey) {
                const char = e.key.toLowerCase();
                if (['c', 'v', 'x', 'a', 'p', 's', 'u'].includes(char)) {
                    e.preventDefault();
                    reportCheatAttempt("restricted_shortcut");
                }
            }
            if (e.key === 'F12') { 
                e.preventDefault(); 
                reportCheatAttempt("devtools_attempt");
            }
        };
        const handleFullScreenChange = () => { 
            if (!document.fullscreenElement) {
                reportCheatAttempt("fullscreen_exit");
            } 
        };
        const handleVisibilityChange = () => { 
            if (document.hidden) {
                reportCheatAttempt("tab_switch");
            }
        };
        
        // Prevent standard clipboard events
    ...
    const handleLayoutSwitch = (mode) => {
        setLayoutMode(mode);
        setShowLayoutModal(false);
        if (mode === 'default') { setSidebarWidth(50); setConsoleSize(30); } 
        else { setSidebarWidth(33.33); setConsoleSize(50); }
    };
    ...
    const executeSubmission = async (isAuto = false) => {
        ...
        try {
            // 1. Submit current problem code (Snapshot)
            const submitResponse = await api.post('/api/student/submit-problem', {
                examId: contestId, 
                problemId: problemId, 
                languageId: LANGUAGE_MAP[languageRef.current], 
            ...
            if (isAuto) {
                // 2. Final Submit (Terminates Exam)
                const finalResponse = await api.post('/api/student/final-submit', { examId: contestId });
                ...
                if (finalResponse.data.success) {
                    ...
                    // If not already showing the violation modal (which is higher priority), show submit modal
                    if (!showViolationModal) setShowSubmitModal(true);
    ...
    // --- TIMER (Updated for 00:05 Auto Submit) ---
    useEffect(() => {
        if (!contestData?.endTime) return;
        const updateTimer = () => {
            const end = new Date(contestData.endTime).getTime();
            ...
            // ⚠️ AUTO-SUBMIT AT 00:05 (5000ms)
            if (distance <= 5000 && distance > 0) {
                if (!isAutoSubmitting.current) {
                    console.log("Time critical (<=5s): Auto submitting...");
    ...
    const handleRun = async () => {
        setStatus('running'); setExecutionResults(null);
        const customTestCasesPayload = testCases.filter(tc => tc.type === 'custom').map(tc => ({ input: String(tc.input), output: tc.output || "" }));
        try {
            const response = await api.post('/api/student/run', {
                examId: contestId, problemId: problemId, languageId: LANGUAGE_MAP[language], code: codeMap[language], customTestCases: customTestCasesPayload 
            });
            const json = response.data;
            if (json.success && json.results) {
                const results = json.results.map((res, index) => {
                    ...
                    if (!originalCase) return null;
                    return {
                        id: originalCase.id, status: res.status, passed: res.passed, input: res.input,
                        expectedOutput: res.expectedOutput, actualOutput: res.actualOutput, stderr: res.stderr, compileOutput: res.compile_output, 
                        time: `${res.time}s`, memory: `${Math.round(res.memory / 1024)}MB` 
                    };
                }).filter(Boolean);
                setExecutionResults(results);
    ...
    const handleMouseDown = (target) => (e) => {
        ...
        const startSidebarW = sidebarWidthRef.current; const startConsoleS = consoleSizeRef.current;
        const onMove = (e) => {
            if (dragTargetRef.current === 'sidebar') setSidebarWidth(Math.min(60, Math.max(20, startSidebarW + ((e.clientX - startX) / window.innerWidth) * 100)));
            else if (dragTargetRef.current === 'console') {
                if (layoutMode === 'default') setConsoleSize(Math.min(85, Math.max(10, startConsoleS + ((startY - e.clientY) / window.innerHeight) * 100)));
                else setConsoleSize(Math.min(60, Math.max(20, startConsoleS + ((startX - e.clientX) / window.innerWidth) * 100)));
            }
    ...
    return (
        <div className={cn("flex flex-col h-screen w-full font-sans overflow-hidden text-sm selection:bg-blue-500/30 relative", theme.appBg, theme.textMain)}>
            ...
            {/* 🚨 VIOLATION ALERT MODAL (UPDATED: NO BLUR, NO ANIMATION) */}
            {showViolationModal && violationData && (
                <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80">
                    <div className={cn("w-full max-w-md p-6 rounded-3xl shadow-2xl border-2 text-center space-y-4", 
                        violationData.action === 'TERMINATE' ? "bg-white border-rose-500" : "bg-white border-amber-500"
                    )}>
                        {/* Icon Container (Animation Removed) */}
                        <div className={cn("w-16 h-16 rounded-full flex items-center justify-center mx-auto", 
                             violationData.action === 'TERMINATE' ? "bg-rose-100 text-rose-600" : "bg-amber-100 text-amber-600"
                        )}>
                            {violationData.action === 'TERMINATE' ? <Ban size={32} /> : <TriangleAlert size={32} />}
                        </div>
                        
                        <div>
                            <h2 className={cn("text-2xl font-black tracking-tight", 
                                violationData.action === 'TERMINATE' ? "text-rose-600" : "text-amber-600"
                            )}>
                                {violationData.action === 'TERMINATE' ? "Exam Terminated" : "Violation Warning"}
                            </h2>
                            <p className="text-slate-600 mt-2 font-medium">{violationData.message}</p>
                        </div>

                        {violationData.action !== 'TERMINATE' && (
                            <div className="py-3 px-4 rounded-xl bg-slate-100 border border-slate-200">
                                <span className="text-slate-500 text-xs font-bold uppercase tracking-wider">Warnings Remaining</span>
                                <div className="text-3xl font-black text-slate-800 mt-1">{violationData.warningsLeft}</div>
                            </div>
                        )}

                        {violationData.action === 'TERMINATE' ? (
                            // Terminate State (Pulse Animation Removed)
                            <div className="flex items-center justify-center gap-2 text-slate-500 text-sm">
                                <Loader2 size={16} className="animate-spin"/> Submitting your answers...
                            </div>
                        ) : (
                            <button onClick={() => setShowViolationModal(false)} className="w-full py-3.5 rounded-xl bg-slate-900 text-white font-bold uppercase tracking-widest hover:bg-slate-800 transition-all">
                                I Understand
                            </button>
                        )}
                    </div>
            ...
            {/* --- LAYOUT SETTINGS MODAL (With REVERTED Preview) --- */}
            {showLayoutModal && (
                <div className={cn("fixed inset-0 z-[200] flex items-center justify-center backdrop-blur-sm animate-in fade-in duration-200", theme.modalOverlay)}>
                    <div className="fixed inset-0" onClick={() => setShowLayoutModal(false)}></div>
                    <div className={cn("w-[500px] rounded-2xl shadow-2xl border overflow-hidden relative z-10", theme.panelBg, theme.border)}>
            ...
            {showSubmitModal && submissionResult && (
                <div className={cn("fixed inset-0 z-[200] flex items-center justify-center backdrop-blur-md animate-in fade-in zoom-in-95 duration-300", theme.modalOverlay)}>
                    <div className={cn("w-full max-w-[480px] rounded-3xl border shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]", theme.panelBg, theme.border)}>
                        <div className={cn("px-6 py-5 border-b flex items-center justify-between shrink-0", theme.border, theme.headerBg)}>
                            <div className="flex items-center gap-3">
                                <div className={cn("w-10 h-10 rounded-full flex items-center justify-center border", isPassed ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-amber-50 border-amber-100 text-amber-600")}>
                                    <Award size={20} className={isPassed ? "fill-current" : ""} />
                                </div>
                                <div>
                                    <h3 className={cn("font-black text-lg tracking-tight", theme.textHead)}>
                                        {isPassed ? "All Tests Passed!" : "Submission Result"}
                                    </h3>
                                    <p className="text-xs font-medium opacity-60">
                                        Score: {submissionResult.marksEarned} / {submissionResult.totalMarks}
                                    </p>
                                </div>
                            </div>
                            <button onClick={() => setShowSubmitModal(false)} className="p-2 rounded-full hover:bg-black/10 transition-colors"><X size={20}/></button>
                        </div>

                        <div className={cn("flex-1 overflow-y-auto custom-scrollbar p-6 space-y-3 bg-opacity-50", theme.isDark ? "bg-black/20" : "bg-slate-50/50")}>
                            <h4 className={cn("text-xs font-black uppercase tracking-widest opacity-50 mb-4 px-1", theme.textHead)}>Test Case Execution</h4>
                            <div className="flex flex-col gap-2">
                                {Array.from({ length: submissionResult.passedCount }).map((_, i) => (
                                    <div key={`pass-${i}`} className={cn("flex items-center justify-between p-3 rounded-xl border bg-white/50 backdrop-blur-sm transition-all hover:scale-[1.01]", theme.border)}>
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={cn("text-xs font-bold", theme.textMain)}>Test Case {i + 1}</span>
                                        </div>
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100">Success</span>
                                    </div>
                                ))}
                                {Array.from({ length: submissionResult.totalCases - submissionResult.passedCount }).map((_, i) => (
                                    <div key={`fail-${i}`} className={cn("flex items-center justify-between p-3 rounded-xl border bg-white/50 backdrop-blur-sm transition-all hover:scale-[1.01]", theme.isDark ? "border-rose-900/30" : "border-rose-100")}>
                                        <div className="flex items-center gap-3">
                                            <div className="w-6 h-6 rounded-full bg-rose-500 flex items-center justify-center text-white shadow-sm shadow-rose-200">
                                                <X size={14} strokeWidth={3} />
                                            </div>
                                            <span className={cn("text-xs font-bold", theme.textMain)}>Test Case {submissionResult.passedCount + i + 1}</span>
                                        </div>
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-rose-600 bg-rose-50 px-2 py-1 rounded-md border border-rose-100">Wrong Answer</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className={cn("p-4 border-t bg-black/5 flex gap-3 shrink-0", theme.border)}>
                            <button onClick={() => setShowSubmitModal(false)} className={cn("flex-1 py-3.5 rounded-xl font-bold uppercase tracking-wider text-xs transition-colors hover:bg-black/5 border border-transparent", theme.textSec)}>
                                Close
                            </button>
                            {(isPassed && !submissionResult.isAuto) && (
                                <button 
                                    onClick={() => navigate(`/contests/${contestId}/live`, { state: { contestData, theme: themeId } })} 
                                    className={cn("flex-1 py-3.5 rounded-xl font-bold uppercase tracking-wider text-xs shadow-lg transition-transform hover:-translate-y-0.5 border border-white/10", theme.accentPrimary)}
                                >
                                    Continue
                                </button>
                            )}
            ...
            <header className={cn("shrink-0 z-20 px-2 pt-2 relative", theme.appBg)}>
                <div className={cn("h-16 rounded-2xl flex items-center justify-between px-6 border shadow-sm backdrop-blur-xl transition-all", theme.headerBg, theme.border)}>
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate(`/contests/${contestId}/live`, { state: { contestData: contestData, theme: themeId } })} className={cn("p-2 rounded-lg border hover:scale-105 active:scale-95 transition-all", theme.panelBg, theme.border, theme.textSec)}><ChevronLeft size={20}/></button>
                        <div><h1 className={cn("font-bold text-base tracking-tight uppercase opacity-80", theme.textHead)}>{contestData?.examName || "Contest"}</h1></div>
                    </div>
                    <div className={cn("absolute left-1/2 -translate-x-1/2 flex items-center gap-2 px-3 py-1.5 rounded-xl border font-mono font-bold transition-all duration-500 text-sm", getTimerStyles())}>
                        <Timer size={14} className={cn(secondsRemaining < 300 && "animate-pulse")} /> <span className="tracking-wider">{timeLeftDisplay}</span>
            ...
            <div className="flex-1 flex overflow-hidden p-2 gap-2">
                ...
                <div style={{ width: `${100 - sidebarWidth}%` }} className={cn("flex h-full gap-2", layoutMode === 'default' ? "flex-col" : "flex-row")}>
                    ...
                    <div className={cn("flex-1 flex flex-col rounded-2xl overflow-hidden border shadow-sm relative", theme.panelBg, theme.border)}>
                        ...
                        <div className="flex-1 relative">
                            <Editor 
                                height="100%" language={language} value={codeMap[language]} onMount={handleEditorDidMount} onChange={handleCodeChange} theme={theme.monaco} 
                                options={{ minimap: { enabled: false }, fontSize, padding: { top: 20, bottom: 80 }, fontFamily: 'JetBrains Mono, monospace', scrollBeyondLastLine: false, smoothScrolling: true, cursorBlinking: 'smooth', cursorSmoothCaretAnimation: 'on', cursorStyle: 'line', contextmenu: false }} 
                            />
                            <div className={cn("absolute bottom-2 left-6 z-10 text-[10px] font-mono opacity-40 pointer-events-none transition-opacity", theme.textMain)}>Ln {cursorPosition.ln}, Col {cursorPosition.col}</div>
                        </div>
                                ...
                                <div className="flex-1 flex flex-col h-full animate-in fade-in duration-200 pt-2">
                                    ...
                                    <div className={cn("w-full relative group transition-all rounded-xl border overflow-hidden p-1 focus-within:ring-2 focus-within:ring-blue-500/50 shadow-inner overflow-y-auto custom-scrollbar", theme.inputBoxHighContrast)}>
                                        <textarea 
                                            ref={el => { if(el) textareaRefs.current[selectedCaseId] = el; }}
                                            value={currentCase?.input || ""} 
                                            readOnly={currentCase?.type === 'sample'} 
                                            onChange={e => { 
                                                const val = e.target.value; 
                                                setTestCases(prev => prev.map(t => t.id === selectedCaseId ? {...t, input: val} : t)); 
                                                adjustTextareaHeight(selectedCaseId);
