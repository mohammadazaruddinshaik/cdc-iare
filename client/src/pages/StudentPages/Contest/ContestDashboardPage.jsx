...
import { 
    ArrowRight, Clock, Terminal, AlertCircle, Loader2, 
    PlayCircle, CheckCircle2, AlertTriangle, X, ShieldAlert, Maximize,
    Ban, TriangleAlert, Wifi, WifiOff 
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
...
const ContestDashboard = () => {
    ...
    const location = useLocation(); 
    const { contestId } = useParams(); 
    const isOnline = useNetworkStatus(); // Hook for connectivity status
    ...
    const [resultModal, setResultModal] = useState({
        show: false, type: 'success', title: '', message: '', onClose: () => {}
    });

    // --- 2. HANDLERS ---
    
    // ✅ Wrapped in useCallback for timer dependency
    const handleFinalSubmit = useCallback(async (isAutoTerminate = false) => {
        // Stop if already submitting, terminated, or offline (unless it's a forced termination where we try anyway)
        if (isSubmitting || (isTerminatedRef.current && !isAutoTerminate)) return;
        
        setIsSubmitting(true);
        if (isAutoTerminate) isTerminatedRef.current = true;
        ...
        try {
            ...
            if (json.success) {
                setResultModal({
                    show: true,
                    type: 'success',
                    title: isAutoTerminate ? 'Exam Auto-Submitted' : 'Submission Successful!',
                    message: json.message || "Your exam has been submitted successfully.",
                    onClose: () => navigate(`/contests/result`, {
                        state: { resultData: json.data, theme: themeId }
                    })
                });
            } else {
                setResultModal({
    ...
    }, [contestId, navigate, themeId, isSubmitting]);

    // --- 3. ANTI-CHEAT & FULL SCREEN LOGIC ---
    const reportCheatAttempt = async (violationType) => {
        if (isSubmitting || isTerminatedRef.current) return;
    ...
    useEffect(() => {
        ...
        const handleVisibilityChange = () => {
            if (document.hidden) {
                setIsLocked(true); 
                reportCheatAttempt("tabswitch");
            }
        };

        const handleContextMenu = (e) => { e.preventDefault(); reportCheatAttempt("contextmenu"); };
        const handleKeyDown = (e) => {
            if (e.key === 'F12' || (e.ctrlKey && ['c','v','x','u','i'].includes(e.key.toLowerCase()))) {
                e.preventDefault();
            }
        };
    ...
    useEffect(() => {
        const fetchExamDetails = async () => {
            try {
                ...
                const response = await api.get(`/api/student/get-exam-details/${contestId}`);
                const json = response.data;
                if (json.success) setContestData(json.data);
                else setError('Failed to load contest details.');
            } catch (err) {
                console.error(err);
                setError('Network error. Please try again.');
            } finally {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (distance <= 0) {
                ...
                // ✅ AUTO-SUBMIT TRIGGER
                if (!isAutoSubmitting.current && !isTerminatedRef.current) {
                    console.log("Time expired. Auto-submitting...");
                    isAutoSubmitting.current = true;
                    handleFinalSubmit(true); // Call submission with auto-flag
                }
            } else {
                const hours = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                const minutes = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
        ...
        return () => clearInterval(interval);
    }, [contestData, handleFinalSubmit]); // Added handleFinalSubmit dependency
    ...
    const handleProblemClick = (problemId) => {
        if (!problemId || !isOnline) return;
        navigate(`/contests/${contestId}/problem/${problemId}`, {
            state: { contestData: contestData, theme: themeId }
        });
    };
    ...
    return (
        <div className={`min-h-screen ${theme.bg} ${theme.text} font-sans selection:bg-indigo-500/30 overflow-x-hidden relative pb-10 transition-colors duration-500`}>
            ...
            {/* 🛡️ SECURITY LOCKOUT MODAL (Updated: No Blur, No Animation, Solid BG) */}
            {(isLocked && !showViolationModal) && (
                <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-gray-900/95">
                    <div className="w-full max-w-md p-8 rounded-3xl bg-white shadow-2xl border border-slate-200 text-center space-y-6">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-slate-900 tracking-tight">Security Lockout</h2>
                            <p className="text-slate-500 mt-2 font-medium">Full screen mode is required to continue.</p>
                        </div>
                        <button onClick={enterFullScreen} className="w-full py-4 rounded-xl bg-slate-900 text-white font-bold uppercase tracking-widest shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
                            <Maximize size={18} /> Enable Full Screen
                        </button>
                    </div>
                </div>
            )}
            ...
            {showViolationModal && violationData && (
                <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 px-4">
                    <div className={`w-full max-w-md p-6 rounded-3xl shadow-2xl border-2 text-center space-y-4 ${
                        ...
                        {violationData.action === 'TERMINATE' ? (
                            ...
                            <button onClick={() => setShowViolationModal(false)} className="w-full py-3.5 rounded-xl bg-slate-900 text-white font-bold uppercase tracking-widest hover:bg-slate-800 transition-all">I Understand</button>
                        )}
                    </div>
                </div>
            )}
            ...
            {/* Background Grid */}
            <div className="fixed inset-0 pointer-events-none">
                {themeId === 'midnight' && <div className="absolute inset-0 bg-gradient-to-br from-[#071225] via-[#0A1B3A] to-[#071225] opacity-50"/>}
            </div>
            ...
            <header className={`sticky top-0 z-50 border-b transition-colors duration-300 ${theme.header}`}>
                <div className="relative max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg ${themeId === 'light' ? 'bg-slate-900 text-white' : 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20'}`}>
                            <Terminal size={20} />
                        </div>
                        <div className="hidden sm:block">
                            <h1 className="font-black text-lg tracking-tight leading-none">{contestData.examName}</h1>
                            <div className="flex items-center gap-2 mt-1">
                                <span className={`w-1.5 h-1.5 rounded-full ${timeLeft === 'ENDED' ? 'bg-red-500' : 'bg-emerald-500 animate-pulse'}`}></span>
                                <span className={`text-[10px] font-bold uppercase tracking-wider ${theme.subtext}`}>
                                    {timeLeft === 'ENDED' ? 'Exam Ended' : 'Live Contest'}
                    ...
                    <div className="flex items-center gap-4">
                        {/* ✅ CONNECTIVITY SYMBOL */}
                        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold uppercase tracking-wider transition-colors ${
                            isOnline 
                                ? (themeId === 'light' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20') 
                                : (themeId === 'light' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-rose-500/10 text-rose-400 border-rose-500/20')
                        }`}>
                        ...
                        <button 
                            ...
                            className={`text-xs font-bold uppercase tracking-wider px-6 py-2.5 rounded-xl shadow-lg transition-all transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                                themeId === 'light' 
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center justify-between w-full sm:w-auto gap-8">
                                                <div className="text-right">
                                                    <span className={`block font-black text-lg ${isSolved ? 'text-emerald-500' : isInProgress ? 'text-amber-500' : ''}`}>
                                                        {prob.marksObtained} <span className="text-base opacity-40"> / {prob.maxMarks}</span>
                                                    </span>
                                                    <span className="text-[9px] font-bold opacity-40 uppercase tracking-widest">Points</span>
                                                </div>
                                                
                                                {isSolved ? (
                                                    <button onClick={() => handleProblemClick(prob.problemId)} className={`h-10 px-6 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 ${themeId === 'dark' ? 'bg-neutral-800 text-emerald-500' : 'bg-emerald-500/10 text-emerald-500'}`}>
                                                        Solved <CheckCircle2 size={14} />
                                                    </button>
                                                ) : isInProgress ? (
                                                    <button onClick={() => handleProblemClick(prob.problemId)} className={`h-10 px-6 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center gap-2 active:scale-95 ${themeId === 'dark' ? 'bg-amber-600 text-white hover:bg-amber-500' : 'bg-amber-500 text-white hover:bg-amber-600 shadow-md shadow-amber-500/20'}`}>
                                                        Resume <PlayCircle size={14} />
                                                    </button>
                                                ) : (
                                                    <button onClick={() => handleProblemClick(prob.problemId)} className={`h-10 px-6 rounded-xl text-xs font-bold uppercase tracking-wider shadow-sm transition-all flex items-center gap-2 group-hover:translate-x-1 ${theme.button}`}>
                                                        Solve <ArrowRight size={14} />
                                                    </button>
                                                )}
                                            </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center text-center space-y-4">
                                ...
                                <div className="flex w-full gap-3 mt-4">
                                    <button onClick={() => setShowFinishModal(false)} className="flex-1 py-3 rounded-xl border font-bold text-sm transition-colors opacity-70 hover:opacity-100">Cancel</button>
                                    <button onClick={() => handleFinalSubmit(false)} disabled={isSubmitting} className={`flex-1 py-3 rounded-xl font-bold text-sm text-white shadow-lg flex items-center justify-center gap-2 ${theme.button}`}>
                                        {isSubmitting ? <Loader2 size={16} className="animate-spin" /> : "Yes, Submit"}
                                    </button>
                                </div>
                            </div>
                    ...
                    >
                        ...
                        >
                            <div className="flex flex-col items-center text-center space-y-4">
                                ...
                                <button onClick={resultModal.onClose} className={`w-full py-3 rounded-xl font-bold text-sm text-white shadow-lg mt-4 ${resultModal.type === 'success' ? 'bg-emerald-600 hover:bg-emerald-500' : 'bg-red-600 hover:bg-red-500'}`}>
                                    {resultModal.type === 'success' ? "Continue" : "Okay"}
                                </button>
                            </div>
                        </motion.div>
