...
// import { useNetworkStatus } from '/src/hooks/Network';
...
const ThemeCard = ({ id, activeTheme, icon: Icon, title, sub, onSelect }) => {
    ...
    return (
        <motion.button
            ...
            whileHover={{ y: -5, scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            // DIMENSION REDUCTION: w-64 -> w-56, h-72 -> h-60, p-6 -> p-5
            className={`
                group relative w-full md:w-56 h-60 rounded-[1.2rem] border text-left p-5 overflow-hidden transition-all duration-300 flex flex-col justify-between backdrop-blur-xl
                ${bgClass} ${hoverBorder} hover:shadow-2xl
            `}
        >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors duration-300 ${iconBg} group-hover:bg-indigo-600 group-hover:text-white group-hover:border-transparent`}>
                <Icon size={20} strokeWidth={1.5} />
            </div>
            <div>
                <h3 className={`text-xl font-black mb-1.5 tracking-tight ${textClass}`}>{title}</h3>
                <p className={`text-[10px] font-medium leading-relaxed ${subClass}`}>{sub}</p>
            </div>
            <div className={`absolute bottom-5 right-5 transition-all duration-300 transform translate-x-4 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 ${isMid || isDark ? 'text-white' : 'text-indigo-600'}`}>
                <ArrowRight size={18} />
            </div>
        </motion.button>
    );
...
const ContestInstructions = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            // Simulates varying latency between 20ms and 60ms
            setLatency(Math.floor(Math.random() * (60 - 20 + 1) + 20));
        }, 3000);
            ...
            <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                    ...
                    >
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
                            <span className="inline-block py-1 px-3 rounded-full bg-slate-100/10 border border-slate-200/20 text-slate-500 backdrop-blur-md text-[10px] font-bold uppercase tracking-widest mb-3 shadow-sm">Environment Setup</span>
                            <h1 className={`text-3xl md:text-4xl font-black tracking-tight mb-2 ${hoveredTheme === 'light' ? 'text-slate-900' : 'text-white'} transition-colors duration-500`}>Choose IDE Theme</h1>
                        </motion.div>
                        
                        {/* THEME CARDS GRID */}
                        <div className="flex flex-col md:flex-row gap-4">
                            <div onMouseEnter={() => setHoveredTheme('light')} onMouseLeave={() => setHoveredTheme(null)}>
                                <ThemeCard id="light" activeTheme={selectedTheme} icon={Sun} title="Light" sub="High contrast. Classic feel." onSelect={handleThemeSelect} />
                            </div>
                            <div onMouseEnter={() => setHoveredTheme('midnight')} onMouseLeave={() => setHoveredTheme(null)}>
                                <ThemeCard id="midnight" activeTheme={selectedTheme} icon={Moon} title="Midnight" sub="Deep blue tones. Reduced strain." onSelect={handleThemeSelect} />
                            </div>
                            <div onMouseEnter={() => setHoveredTheme('dark')} onMouseLeave={() => setHoveredTheme(null)}>
                                <ThemeCard id="dark" activeTheme={selectedTheme} icon={Zap} title="Dark" sub="Pure black. Distraction free." onSelect={handleThemeSelect} />
                            </div>
                ...
                {step === 2 && (
                    ...
                    >
                        ...
                        <div className="w-full max-w-4xl px-6 py-6 relative z-30 h-full overflow-y-auto no-scrollbar flex flex-col justify-center">
                            {/* Header */}
                            <div className="flex items-end justify-between mb-6">
                                <div>
                                    <div className={`flex items-center gap-2 mb-1 text-[10px] font-mono uppercase tracking-widest ${ts.accentColor}`}>
                                        <Terminal size={12} /> <span>Protocol Briefing</span>
                                    </div>
                                    <h1 className={`text-2xl md:text-3xl font-black tracking-tight leading-none ${ts.text}`}>{config.title}</h1>
                                </div>
                                <div className="hidden md:block text-right">
                                    <div className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${ts.subText}`}>System Status</div>
                                    <div className="flex items-center gap-2 justify-end">
                                        ...
                                        <span className={`${isOnline ? 'text-emerald-500' : 'text-red-500'} font-bold text-xs`}>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
                                <motion.div 
                                    ...
                                    animate={{ opacity: 1, y: 0 }} 
                                    transition={{ delay: 0.1 }} 
                                    // DIMENSION REDUCTION: Rounded corners
                                    className={`lg:col-span-2 rounded-[1.2rem] border overflow-hidden backdrop-blur-xl ${ts.cardBg} ${ts.cardBorder}`}
                                >
                                    <div className={`grid grid-cols-3 divide-x h-full ${ts.divider}`}>
                                        {/* Reduced Padding: p-4 */}
                                        <div className="p-4 flex flex-col justify-center items-center text-center">
                                            <div className={`mb-2 p-1.5 rounded-lg ${ts.iconBg}`}><Clock size={18} /></div>
                                            <div className={`text-[9px] font-bold uppercase tracking-wider mb-0.5 opacity-60 ${ts.subText}`}>Time Limit</div>
                                            <div className={`font-bold text-base ${ts.text}`}>{config.durationStr}</div>
                                        </div>
                                        <div className="p-4 flex flex-col justify-center items-center text-center">
                                            <div className={`mb-2 p-1.5 rounded-lg ${selectedTheme === 'dark' ? 'bg-neutral-800 text-white' : 'bg-purple-500/10 text-purple-500'}`}><Code2 size={18} /></div>
                                            <div className={`text-[9px] font-bold uppercase tracking-wider mb-0.5 opacity-60 ${ts.subText}`}>Problems</div>
                                            <div className={`font-bold text-base ${ts.text}`}>{config.problems}</div>
                                        </div>
                                        <div className="p-4 flex flex-col justify-center items-center text-center">
                                            <div className={`mb-2 p-1.5 rounded-lg ${selectedTheme === 'dark' ? 'bg-neutral-800 text-white' : 'bg-amber-500/10 text-amber-500'}`}><Trophy size={18} /></div>
                                            <div className={`text-[9px] font-bold uppercase tracking-wider mb-0.5 opacity-60 ${ts.subText}`}>Total Score</div>
                                            <div className={`font-bold text-base ${ts.text}`}>{config.score}</div>
                                        </div>
                                    </div>
                                </motion.div>

                                {/* System Check (Dynamic) */}
                                <motion.div 
                                    initial={{ opacity: 0, y: 10 }} 
                                    animate={{ opacity: 1, y: 0 }} 
                                    transition={{ delay: 0.2 }} 
                                    className={`p-4 rounded-[1.2rem] border flex flex-col justify-between backdrop-blur-xl ${ts.cardBg} ${ts.cardBorder}`}
                                >
                                    <div className={`text-[10px] font-bold uppercase tracking-widest mb-3 flex items-center gap-2 ${ts.subText}`}><AlertOctagon size={14} /> Environment</div>
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between text-xs">
                                            <span className={`font-mono opacity-60 ${ts.subText}`}>Latency</span>
                                            <span className={`font-bold ${latency < 50 ? 'text-emerald-500' : 'text-yellow-500'}`}>{latency}ms</span>
                                        </div>
                                        <div className="flex items-center justify-between text-xs">
                                            <span className={`font-mono opacity-60 ${ts.subText}`}>Anti-Cheat</span>
                                            <span className="font-bold text-emerald-500">ACTIVE</span>
                                        </div>
                                        <div className="flex items-center justify-between text-xs">
                                            <span className={`font-mono opacity-60 ${ts.subText}`}>Network</span>
                                            {isOnline ? (
                            ...
                            <motion.div 
                                ...
                                transition={{ delay: 0.3 }} 
                                // Reduced padding: p-4
                                className={`mb-6 p-4 rounded-[1.2rem] border backdrop-blur-md ${ts.rulesBg}`}
                            >
                                <div className="flex items-center gap-2 mb-3 text-red-500 font-bold uppercase text-[10px] tracking-widest">
                                    <ShieldAlert size={14} /> Zero Tolerance Policy
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    {[
                                        { t: "Tab Switching", d: "Focus loss is logged.", i: MonitorX },
                                        { t: "Plagiarism", d: "MOSS AI Check Enabled.", i: Eye },
                                        { t: "AI Tools", d: "Extensions blocked.", i: Lock }
                                    ].map((rule, idx) => (
                                        <div key={idx} className={`flex gap-2 items-start ${ts.rulesText}`}>
                                            <rule.i size={14} className="mt-0.5 shrink-0 opacity-70" />
                                            <div><div className="text-[10px] font-bold uppercase mb-0.5">{rule.t}</div><div className="text-[10px] leading-tight opacity-80">{rule.d}</div></div>
                                        </div>
                                    ))}
                                </div>
                                
                                {/* --- NEW WARNING LIMIT --- */}
                                <div className={`mt-3 pt-3 border-t ${selectedTheme === 'light' ? 'border-red-200' : 'border-red-500/20'} flex items-center gap-2 text-red-600`}>
                                    <Activity size={14} />
                                    <span className="text-[10px] md:text-xs font-bold uppercase">
                                        Warning Limit: Max 4 warnings allowed. 
                                        <span className="opacity-70 normal-case ml-1">Exceeding this limit will result in immediate auto-submission.</span>
                                    </span>
                                </div>
                            </motion.div>
                            ...
                            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="space-y-4">
                                <label className="flex items-center justify-center gap-2 cursor-pointer group">
                                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center transition-all ${agreed ? 'bg-indigo-600 border-indigo-600' : `border-slate-400 group-hover:border-indigo-400`}`}>
                                        {agreed && <Check size={10} className="text-white" strokeWidth={4} />}
                                    </div>
                                    <input type="checkbox" className="hidden" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} />
                                    <span className={`text-xs font-medium select-none ${selectedTheme === 'light' ? 'text-slate-600 group-hover:text-slate-800' : 'text-slate-400 group-hover:text-white'}`}>I accept the Rules of Engagement and integrity protocols.</span>
                                </label>

                                <div className={`relative h-14 rounded-full overflow-hidden border transition-all duration-300 
                                    ${ts.sliderTrack} ${ts.cardBorder}
                                    ${!agreed ? 'opacity-50 grayscale cursor-not-allowed' : 'opacity-100'}
                                `}>
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                                        <span className={`text-[10px] font-black uppercase tracking-[0.2em] transition-opacity duration-300 ${sliderVal > 30 ? 'opacity-0' : 'opacity-40'} ${selectedTheme === 'light' ? 'text-slate-400' : 'text-white'}`}>{agreed ? "Slide to Initialize" : "Accept Rules to Unlock"}</span>
                                    </div>
                                    <div className={`absolute inset-y-0 left-0 transition-none ${ts.sliderFill}`} style={{ width: `${sliderVal}%` }} />
                                    <input type="range" min="0" max="100" value={sliderVal} onChange={handleSlider} disabled={!agreed} className="range-slider absolute inset-0 w-full h-full opacity-0 cursor-pointer z-30" />
                                    <div className={`absolute top-1.5 bottom-1.5 w-12 rounded-full flex items-center justify-center pointer-events-none z-20 transition-transform ease-out bg-indigo-600 text-white shadow-lg shadow-indigo-500/30`} style={{ left: `calc(${sliderVal}% - ${sliderVal * 0.45}px + 6px)`, transform: `translateX(-${sliderVal}%)` }}>
                                        {sliderVal > 90 ? <Cpu size={18} className="animate-pulse" /> : <ChevronRight size={18} strokeWidth={3} />}
                                    </div>
                                </div>
                            </motion.div>
                        </div>
