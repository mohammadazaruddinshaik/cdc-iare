...
const LiveTimer = ({ startTime, endTime }) => {
    ...
    return (
        <div className="text-right">
            <p className="text-[10px] font-bold text-slate-400 uppercase leading-tight">{timerData.status}</p>
            <p className="font-mono font-bold text-indigo-600 text-xs sm:text-sm">{timerData.time}</p>
        </div>
    );
};
...
const DataStreamBackground = () => {
    ...
    return (
        <div className="fixed inset-0 overflow-hidden pointer-events-none bg-slate-50 selection:bg-none">
            <div className="absolute inset-0 bg-gradient-to-r from-slate-100 via-white to-slate-100 opacity-80"></div>
            {/* Reduced blur size */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-indigo-50/50 rounded-full blur-[80px] opacity-60"></div>
            <div className="absolute left-2 top-0 bottom-0 w-32 overflow-hidden opacity-10 hidden md:block">
                <motion.div animate={{ y: [0, -1000] }} transition={{ duration: 40, repeat: Infinity, ease: "linear" }} className="flex flex-col gap-12 text-right font-black text-[10px] tracking-[0.2em] text-slate-400">
                    {[...leftPhrases, ...leftPhrases, ...leftPhrases, ...leftPhrases].map((item, i) => (
                        <span key={`l-${i}`} className="whitespace-nowrap">{item}</span>
                    ))}
                </motion.div>
            </div>
            <div className="absolute right-2 top-0 bottom-0 w-32 overflow-hidden opacity-10 hidden md:block">
                <motion.div animate={{ y: [-1000, 0] }} transition={{ duration: 50, repeat: Infinity, ease: "linear" }} className="flex flex-col gap-12 text-left font-black text-lg tracking-tighter text-indigo-300">
                    {[...rightPhrases, ...rightPhrases, ...rightPhrases, ...rightPhrases].map((item, i) => (
                        <span key={`r-${i}`} className="whitespace-nowrap">{item}</span>
                    ))}
                </motion.div>
...
const ContestEntry = () => {
    ...
    return (
        <div className="relative min-h-screen w-full flex flex-col items-center justify-center font-sans text-slate-900 overflow-hidden p-4">
            <DataStreamBackground />

            {/* REDUCED MAX-WIDTH: Changed max-w-[580px] to max-w-md (approx 448px) */}
            <div className="relative z-10 w-full max-w-md">
                <motion.div layout className="text-center mb-8">
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="inline-block mb-3 px-3 py-1 rounded-full bg-slate-900 text-white text-[10px] font-bold uppercase tracking-[0.2em]">
                        Not Just Code
                    </motion.div>
                    
                    {/* REDUCED TITLE SIZE: text-5xl/6xl -> text-3xl/4xl */}
                    <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-slate-900 mb-2 leading-[0.95]">
                        It’s a Battle <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-500">
                            Of Logic.
                        </span>
                ...
                <motion.div 
                    layout
                    className={`
                        relative bg-white/80 backdrop-blur-xl rounded-[2rem] transition-all duration-500
                        ${status === 'SUCCESS' ? 'shadow-[0_20px_50px_-12px_rgba(79,70,229,0.3)] ring-2 ring-indigo-50 border-indigo-100' : 
                          status === 'ERROR' ? 'shadow-xl shadow-red-200/50 ring-2 ring-red-50 border-red-100' : 
                          'shadow-xl shadow-slate-200/80 border border-white'}
                    `}
                >
                    ...
                    <div className="p-2">
                        <div className="relative group">
                            <input
                                ...
                                placeholder="ENTER CODE"
                                maxLength={6}
                                // REDUCED INPUT HEIGHT & FONT: h-24 -> h-16, text-3xl -> text-2xl
                                className={`
                                    w-full h-16 text-center text-2xl font-black tracking-widest rounded-[1.7rem] outline-none transition-all duration-300 font-mono uppercase
                                    placeholder:font-sans placeholder:text-slate-200 placeholder:text-lg placeholder:tracking-widest placeholder:font-bold
                                    ${status === 'LOADING' ? 'bg-indigo-50/50 text-indigo-600' : 'bg-white hover:bg-slate-50/50 text-slate-800'}
                                    ${status === 'ERROR' ? 'text-red-500 bg-red-50/50' : ''}
                                `}
                            />
                            ...
                            <div className="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer" onClick={() => verifyExamId(examId)}>
                                <AnimatePresence mode="wait">
                                    {status === 'IDLE' && (
                                        <motion.div key="idle" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="bg-slate-100 p-2 rounded-full text-slate-400 hover:bg-slate-200 hover:text-slate-600 transition-colors">
                                            <ArrowRight size={20} />
                                        </motion.div>
                                    )}
                                    {status === 'LOADING' && (
                                        <motion.div key="loading" initial={{ scale: 0 }} animate={{ scale: 1, rotate: 360 }} exit={{ scale: 0 }} transition={{ rotate: { duration: 1, repeat: Infinity, ease: "linear" } }} className="bg-indigo-100 p-2 rounded-full text-indigo-600 border border-indigo-200">
                                            <Search size={20} strokeWidth={3} />
                                        </motion.div>
                                    )}
                                    {status === 'SUCCESS' && (
                                        <motion.div key="success" initial={{ scale: 0, rotate: -90 }} animate={{ scale: 1, rotate: 0 }} className="bg-emerald-500 p-2 rounded-full text-white shadow-lg shadow-emerald-200">
                                            <CheckCircle2 size={20} strokeWidth={3} />
                                        </motion.div>
                                    )}
                                    {status === 'ERROR' && (
                                        <motion.div key="error" initial={{ scale: 0, rotate: 90 }} animate={{ scale: 1, rotate: 0 }} className="bg-red-500 p-2 rounded-full text-white shadow-lg shadow-red-200">
                    ...
                    <AnimatePresence>
                        {status === 'ERROR' && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden text-center pb-3 px-6">
                                <p className="text-xs font-bold text-red-500 bg-red-50 py-1.5 rounded-lg border border-red-100">{errorMsg}</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    <AnimatePresence>
                        {status === 'SUCCESS' && apiData && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ type: "spring", bounce: 0.3 }} className="overflow-hidden">
                                {/* REDUCED CONTENT PADDING: px-8 pb-8 -> px-6 pb-6 */}
                                <div className="px-6 pb-6 pt-1">
                                    <div className="w-full h-px bg-slate-100 mb-4"></div>
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="pr-2">
                                            <h2 className="text-lg font-black text-slate-800 tracking-tight leading-none mb-1.5">{apiData.examName}</h2>
                                            <div className="flex items-center gap-1.5 bg-yellow-50 px-2 py-0.5 rounded text-yellow-700 w-fit">
                                                <AlertCircle size={10} />
                                                <p className="text-[9px] font-bold uppercase tracking-wide">{apiData.message}</p>
                                            </div>
                                        </div>
                                        <LiveTimer startTime={apiData.startTime} endTime={apiData.endTime} />
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mb-4">
                                        <div className="flex items-center gap-2.5 p-2.5 bg-white rounded-lg border border-slate-100 shadow-sm">
                                            <div className="p-1.5 bg-blue-50 text-blue-600 rounded-md"><Clock size={16} /></div>
                                            <div>
                                                <p className="text-[9px] font-bold text-slate-400 uppercase">Duration</p>
                                                <p className="text-xs font-bold text-slate-900">{getDurationString(apiData.startTime, apiData.endTime)}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2.5 p-2.5 bg-white rounded-lg border border-slate-100 shadow-sm">
                                            <div className="p-1.5 bg-purple-50 text-purple-600 rounded-md"><FileQuestion size={16} /></div>
                                            <div>
                                                <p className="text-[9px] font-bold text-slate-400 uppercase">Problems</p>
                                                <p className="text-xs font-bold text-slate-900">{apiData.problemsCount}</p>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="flex flex-wrap gap-2 mb-6">
                                        <div className="px-2 py-1 bg-slate-100 rounded text-[10px] font-bold text-slate-600 flex items-center gap-1.5">
                                            <Layers size={10} /> Sem: {apiData.semester}
                                        </div>
                                        {apiData.batches.map((batch, idx) => (
                                            <div key={idx} className="px-2 py-1 bg-indigo-50 border border-indigo-100 rounded text-[10px] font-bold text-indigo-600">
                                                {batch}
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <motion.button
                                        ...
                                        className="w-full py-3 bg-slate-900 hover:bg-indigo-600 text-white font-bold text-xs uppercase tracking-widest rounded-xl shadow-lg shadow-slate-200 hover:shadow-indigo-300 transition-all duration-300 flex items-center justify-center gap-2 group"
                                    >
                                        <span>View Instructions</span>
