...
//     GraduationCap, FolderOpen, Copy, AlertTriangle
// } from 'lucide-react';
...
//                                 <p className="text-lg font-mono font-bold text-gray-800">{data.password}</p>
//                             </div>
//                             <button 
...
//                                     <div className="flex items-center gap-4 mb-5">
//                                         <div className="relative">
//                                             <img className="w-14 h-14 rounded-full object-cover border-2 border-white shadow-md bg-gray-100 group-hover:scale-105 transition-transform" src={`https://iare-data.s3.ap-south-1.amazonaws.com/uploads/STUDENTS/${safeRoll}/${safeRoll}.jpg`} onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${safeName.replace(/ /g, '+')}&background=F3F4F6&color=111827&font-size=0.45&rounded=true`; }} alt={safeName} />
//                                         </div>
//                                         <div className="min-w-0">
...
//                         <img className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg mx-auto bg-gray-200" src={`https://iare-data.s3.ap-south-1.amazonaws.com/uploads/STUDENTS/${editData.rollno}/${editData.rollno}.jpg`} onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${editData.name.replace(/ /g, '+')}&background=F3F4F6&color=111827&font-size=0.45&rounded=true`; }} alt={editData.name}/>
...
const ModifyStudentPanel = ({ animate, preloadedStudent, onCancel, onStudentUpdated, currentSemester }) => {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            await response.json(); // Standard response (usually just success message, decryption optional if sensitive)
            onStudentUpdated(true, 'Student updated successfully!'); 
            onCancel();
        } catch (error) { 
            onStudentUpdated(false, error.message || 'Failed to update student.'); 
...
const ManageStudentPage = () => {
    ...
    const handleStudentDelete = async (rollno) => {
        try {
            // DELETE Request: Encrypt Body
            const encryptedBody = encryptData({ semname: currentSemester, rollno: rollno });
    ...
    const handlePasswordReset = async (username) => {
        try {
            // PATCH Request: Encrypt Body
            const encryptedBody = encryptData({ semname: currentSemester, username: username });
