...
const ViewAllFaculty = ({ animate, facultyList, isLoading, error, onAction, onDelete, onResetPassword }) => {
    ...
    const renderContent = () => {
        // --- UPDATED: SKELETON LOADING GRID (Lazy Load Look) ---
        if (isLoading) {
            return (
...
const AddFacultyForm = ({ animate, onCancel, onFacultyAdded }) => {
    ...
    const resetForm = () => {
        ...
        setSelectedSems([]);
        setAssignments({});
        // Note: semData is cached, so we don't need to clear it necessarily
    };
    ...
    // Use the new FacultyOnboardModal here instead of the generic one
    if(successData) {
        return <FacultyOnboardModal 
            ...
            onClose={() => {
                ...
                // Notify parent to refresh list, but keep message specific to "Next" action if you want, or just generic
                onFacultyAdded(true, 'Faculty added! Ready for next entry.'); 
                // REMOVED: onCancel() call, so the user stays on the add tab
            }} 
        />;
    }
                                        ...
                                        <div className="p-4">
                                            {isLoading ? <div className="text-center py-6 text-gray-400 text-sm">Fetching courses...</div> : data ? (
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                                    ...
                                                    }) : <div className="col-span-2 text-center text-sm text-gray-400 italic">No courses available for this semester.</div>}
                                                </div>
    ...
    const handleUpdate = async (e) => {
        ...
        Object.keys(assignments).forEach(sem => {
            ...
            Object.keys(courses).forEach(course => {
                const batches = courses[course];
                batches.forEach(batch => {
                    // Push Subject Assignment: "VI,SN1:CSM601"
                    subjects_assigned.push(`${sem},${batch}:${course}`);
                    // Push Batch Assignment: "VI:SN1"
                    batches_assigned.push(`${sem}:${batch}`);
                });
            });
        });
    ...
    return (
        <div className="min-h-screen font-sans bg-[#F3F4F6] pb-10">
            ...
            <div className="bg-[#0F172A] pb-32 rounded-b-[3rem] shadow-2xl relative overflow-hidden">
                ...
                <div className="px-6 pt-6 relative z-10 max-w-7xl mx-auto">
                    ...
                    <div className="mt-8 mb-6">
                        <SectionHeader title="Faculty Management" animate={animate} delay={200} />
            ...
            <main className="px-4 -mt-24 relative z-20 max-w-7xl mx-auto">
                ...
                {/* CONDITIONAL RENDERING FOR RESETTING STATE */}
                {activeTab === 'add' && (
                    <div className="block">
                ...
                <div className={activeTab === 'find' ? 'block' : 'hidden'}>
                    {preloadedFaculty ? (
                        ...
                        <div className="bg-white rounded-3xl p-10 shadow-xl border border-gray-100 text-center animate-in fade-in zoom-in-95 flex flex-col items-center justify-center min-h-[400px]">
                            ...
                            <p className="text-gray-500 mb-6 font-medium max-w-sm">Select a faculty member from the <b>Directory</b> to edit their profile details or update assignments.</p>
