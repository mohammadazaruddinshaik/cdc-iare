...
// // --- UPDATED MODIFY COLLECTION MODAL ---
// const ModifyCollectionModal = ({ collection, onClose, onSuccess }) => {
//     const [loading, setLoading] = useState(false);
//     const [configLoading, setConfigLoading] = useState(false);
//     const [courseList, setCourseList] = useState([]); // Stores the active list of courses
//     const [newCourseInput, setNewCourseInput] = useState("");
//     const [status, setStatus] = useState({ type: null, message: null });

//     // Extract Sem and Batch from name (e.g., "VI-SEM-attendance-SB1")
...
//             // Logic to find the specific batch config
...
//                                                 className="ml-1 text-gray-400 hover:text-red-500 transition-colors p-0.5 rounded-md hover:bg-red-50"
//                                             >
//                                                 <X size={14} />
//                                             </button>
//                                         </div>
...
//                     <button 
//                         onClick={handleSave} 
//                         disabled={loading || configLoading || courseList.length === 0}
//                         className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-bold shadow-lg hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all"
//                     >
...
//                             {!isUploading && (
//                                 <button onClick={() => setConfirmAction(null)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600"><X size={20}/></button>
//                             )}
//                             <h3 className="text-xl font-bold text-gray-900 mb-2 capitalize">Confirm {confirmAction}</h3>
//                             <p className="text-gray-500 text-sm mb-4">
//                                 Type: <span className="font-mono font-bold text-gray-800 bg-gray-100 px-2 py-1 rounded">{validationTarget}</span>
//                             </p>
...
//                     Permanently delete <strong>{collectionName}</strong>? <br/>
//                     Type <span className="font-bold text-red-600">"delete"</span> to confirm.
//                 </p>
//                 <input 
//                     type="text" placeholder="Type delete..." value={confirmInput} onChange={(e) => setConfirmInput(e.target.value)} disabled={isDeleting}
...
//                     disabled={confirmInput.toLowerCase() !== "delete" || isDeleting}
//                     className={`w-full py-3 rounded-xl font-bold text-white transition-all flex items-center justify-center gap-2 ${confirmInput.toLowerCase() === "delete" ? 'bg-red-600 hover:bg-red-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
//                 >
...
//     const [modifyOperation, setModifyOperation] = useState('add'); // 'add', 'remove', 'reset'
//     const [renameInput, setRenameInput] = useState("");
    
//     // Modify Batches State
//     const [newBatchInput, setNewBatchInput] = useState("");
//     const [selectedModifyBatches, setSelectedModifyBatches] = useState([]); // For remove/reset
//     const [batchesToAdd, setBatchesToAdd] = useState([]); // For add

//     // Modals
//     const [selectedCollection, setSelectedCollection] = useState(null); // For Upload/Manage
//     const [selectedModifyCollection, setSelectedModifyCollection] = useState(null); // For Modify (New)
...
//     const handleLocalDeleteSuccess = (deletedName) => {
//         setFlatList(prevList => prevList.filter(item => item.collectionName !== deletedName));
...
//             // UPDATED DATA PARSING for { success, data: { batches, courses, isActive } }
...
//         if(b && !batchesToAdd.includes(b)) {
//             setBatchesToAdd(prev => [...prev, b]);
//         }
//         setNewBatchInput("");
//     };

//     const toggleModifyBatchSelection = (batchName) => {
//         setSelectedModifyBatches(prev => 
...
//                 payload.batches = batchesToAdd;
//             } else {
//                 payload.batches = selectedModifyBatches;
//             }
...
//         if (col.collectionName.toLowerCase().includes('attendance')) {
...
//                         <button onClick={() => navigate(-1)} className="group flex items-center text-slate-400 hover:text-white transition-colors mb-6 text-sm font-medium">
...
//                                 <button onClick={() => handleTabChange('create')} className={`px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeMainTab === 'create' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}>
//                                     <PlusCircle size={16} /> Create Sem
//                                 </button>
//                                 <button onClick={() => handleTabChange('modify')} className={`px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeMainTab === 'modify' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}>
//                                     <Settings size={16} /> Modify Sem
//                                 </button>
//                                 <button onClick={() => handleTabChange('collections')} className={`px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-all ${activeMainTab === 'collections' ? 'bg-white text-slate-900 shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}>
...
//                                             {SEMESTERS.map(sem => (
//                                                 <button key={sem} onClick={() => setNewSem(sem)}
//                                                     className={`py-3 rounded-lg text-sm font-bold border transition-all ${newSem === sem ? 'border-blue-600 bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'border-gray-200 bg-white text-gray-600 hover:border-blue-300 hover:bg-blue-50'}`}>
//                                                     {sem}
//                                                 </button>
//                                             ))}
//                                         </div>
...
//                                                 return (
//                                                     <button key={batch} onClick={() => toggleBatch(batch)}
//                                                         className={`px-3 py-2.5 rounded-lg text-xs font-bold border transition-all flex items-center justify-center gap-2 ${getBatchColorStyle(batch, isActive)}`}>
...
//                                     </div>
//                                     <div className="pt-8">
//                                             <button onClick={handleInitSubmit} disabled={!newSem || selectedBatches.length === 0 || isInitSubmitting}
//                                                 className="w-full md:w-auto px-10 py-3 bg-gray-900 hover:bg-black text-white rounded-xl font-bold shadow-xl hover:-translate-y-1 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
...
//                                             <LayoutDashboard className="text-blue-600" size={18} /> Configuration Preview
//                                         </h3>
...
//                                                         <span key={b} className={`px-3 py-1.5 rounded-lg text-sm font-bold border flex items-center gap-1 ${getBatchColorStyle(b, true)}`}>
//                                                             <Grid size={12} className="opacity-50"/> {b}
//                                                         </span>
...
//                 {/* === VIEW 2: MODIFY SEMESTER (UPDATED) === */}
//                 {activeMainTab === 'modify' && (
//                     <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden transition-all duration-500 animate-in fade-in slide-in-from-bottom-8">
//                         {!modifySemName ? (
//                             <div className="p-10 min-h-[500px] flex flex-col items-center justify-center">
//                                 <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
//                                     <Settings className="text-indigo-600" /> Select Semester to Modify
//                                 </h2>
//                                 <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full max-w-2xl">
//                                     {SEMESTERS.map(sem => (
//                                         <button key={sem} onClick={() => fetchSemConfig(sem)} disabled={modifyLoading}
//                                             className="group py-6 rounded-2xl border-2 border-gray-100 hover:border-indigo-500 hover:bg-indigo-50 transition-all flex flex-col items-center justify-center gap-2">
...
//                                     <button onClick={() => setModifySemName(null)} className="mb-6 text-xs font-bold text-gray-500 hover:text-gray-800 flex items-center gap-1">
//                                         <ArrowLeft size={12} /> Select Different Semester
//                                     </button>
...
//                                                             <div key={i} className="px-3 py-1 bg-gray-100 rounded-md border border-gray-200 text-xs font-bold text-gray-600 flex items-center gap-2">
//                                                                 {batch}
//                                                             </div>
...
//                                                             <div key={i} className="px-3 py-1 bg-blue-50 rounded-md border border-blue-100 text-xs font-bold text-blue-600 flex items-center gap-2">
...
//                                             <Edit3 size={16} /> Rename Semester
//                                         </h3>
//                                         <div className="flex gap-2">
//                                             <input type="text" placeholder="New Name (e.g. VII)" value={renameInput} onChange={(e) => setRenameInput(e.target.value)} 
//                                                 className="flex-grow px-4 py-2 border border-gray-300 rounded-xl outline-none focus:border-indigo-500 text-sm font-bold"
//                                             />
//                                             <button onClick={() => executeModify('rename')} disabled={modifyActionLoading} 
//                                                 className="px-4 py-2 bg-gray-900 text-white rounded-xl font-bold text-sm hover:bg-black disabled:opacity-50">
//                                                 Save
//                                             </button>
//                                         </div>
//                                     </div>
...
//                                             <button key={op} onClick={() => { setModifyOperation(op); setSelectedModifyBatches([]); setBatchesToAdd([]); }}
//                                                 className={`flex-1 py-2 rounded-lg text-xs font-bold capitalize transition-all ${modifyOperation === op ? 'bg-white shadow-sm text-gray-900' : 'text-gray-400 hover:text-gray-600'}`}>
//                                                 {op} Batches
//                                             </button>
//                                         ))}
//                                     </div>
...
//                                                         {batchesToAdd.map(b => (
//                                                             <span key={b} className="px-3 py-1.5 bg-white text-indigo-600 font-bold rounded-lg border border-indigo-200 shadow-sm flex items-center gap-2">
//                                                                 {b} <button onClick={() => setBatchesToAdd(prev => prev.filter(x => x!==b))}><X size={12}/></button>
//                                                             </span>
//                                                         ))}
//                                                         {batchesToAdd.length === 0 && <span className="text-indigo-300 text-sm italic">No batches added yet</span>}
//                                                     </div>
//                                                 </div>
//                                                 <button onClick={() => executeModify('add')} disabled={modifyActionLoading || batchesToAdd.length === 0} 
...
//                                                     {modifyActionLoading ? <Loader2 className="animate-spin"/> : <Save size={18} />} Save Changes
//                                                 </button>
//                                             </div>
//                                         )}
...
//                                                             return (
//                                                                 <button key={batch} onClick={() => toggleModifyBatchSelection(batch)}
//                                                                     className={`px-4 py-2 rounded-lg font-bold border-2 transition-all flex items-center gap-2 ${isSel ? 'bg-red-500 border-red-500 text-white' : 'bg-white border-red-100 text-gray-500 hover:border-red-300'}`}>
//                                                                     {isSel ? <Trash2 size={14}/> : <MinusCircle size={14}/>} {batch}
//                                                                 </button>
//                                                             )
//                                                         })}
//                                                     </div>
//                                                 </div>
//                                                 <button onClick={() => executeModify('remove')} disabled={modifyActionLoading || selectedModifyBatches.length === 0} 
//                                                     className="w-full py-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-2">
//                                                     {modifyActionLoading ? <Loader2 className="animate-spin"/> : <Trash2 size={18} />} Remove Selected
//                                                 </button>
//                                             </div>
//                                         )}
...
//                                                             return (
//                                                                 <button key={batch} onClick={() => toggleModifyBatchSelection(batch)}
//                                                                     className={`px-4 py-2 rounded-lg font-bold border-2 transition-all flex items-center gap-2 ${isSel ? 'bg-orange-500 border-orange-500 text-white' : 'bg-white border-orange-100 text-gray-500 hover:border-orange-300'}`}>
//                                                                     {isSel ? <RefreshCw size={14}/> : <RefreshCw size={14} className="opacity-50"/>} {batch}
//                                                                 </button>
//                                                             )
//                                                         })}
//                                                     </div>
//                                                 </div>
//                                                 <button onClick={() => executeModify('reset')} disabled={modifyActionLoading || selectedModifyBatches.length === 0} 
//                                                     className="w-full py-4 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold shadow-lg transition-all flex items-center justify-center gap-2">
//                                                     {modifyActionLoading ? <Loader2 className="animate-spin"/> : <RefreshCw size={18} />} Reset Attendance
//                                                 </button>
//                                             </div>
//                                         )}
//                                     </div>
...
//                                 <p className="text-gray-500 font-medium">Fetching database collections...</p>
//                             </div>
//                         ) : (
//                             <>
...
//                                             {/* --- UPDATED GRID LAYOUT FOR WIDER BOXES --- */}
...
//                                                             className={`group relative rounded-xl p-5 border transition-all duration-300 ${theme.classes} ${isAttendance ? 'cursor-pointer hover:-translate-y-1' : 'cursor-not-allowed opacity-80'}`}
//                                                         >
//                                                             {/* --- DELETE BUTTON (Visible only if NOT 'Others') --- */}
//                                                             {groupName !== 'Others' && (
//                                                                 <button 
//                                                                     onClick={(e) => handleDeleteClick(e, col.collectionName)}
//                                                                     className="absolute top-4 right-4 p-2 bg-red-100 text-red-500 hover:bg-red-500 hover:text-white rounded-lg border border-red-200 transition-colors shadow-sm"
...
//                                                                 <div className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm ${theme.iconColor}`}>
//                                                                     <theme.icon size={20} />
//                                                                 </div>
//                                                                 <span className={`text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wide border ${theme.badge}`}>
//                                                                     {theme.label}
//                                                                 </span>
//                                                             </div>
//                                                             <div className="mb-4 h-12">
//                                                                 <h3 className="text-base font-bold text-gray-800 leading-snug line-clamp-2 group-hover:text-blue-600 transition-colors">
//                                                                     {col.collectionName}
//                                                                 </h3>
//                                                             </div>
...
//                                                                 {/* --- ACTION BUTTONS (Update & Modify) --- */}
//                                                                 {isAttendance && (
//                                                                     <div className="flex gap-2">
//                                                                         {/* MODIFY BUTTON */}
//                                                                         <button 
//                                                                             onClick={(e) => handleModifyCollectionClick(e, col)}
//                                                                             className="text-[10px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 rounded-full flex items-center gap-1 shadow-md transition-all"
//                                                                         >
//                                                                             <Edit3 size={10} /> Modify
//                                                                         </button>

//                                                                         {/* UPLOAD BUTTON (Existing) */}
//                                                                         <span className="text-[10px] font-bold text-white bg-black hover:bg-gray-800 px-3 py-1.5 rounded-full flex items-center gap-1 shadow-md transition-all">
...
//             {/* NEW MODIFY COLLECTION MODAL */}
//             {selectedModifyCollection && (
//                 <ModifyCollectionModal 
//                     collection={selectedModifyCollection} 
//                     onClose={() => setSelectedModifyCollection(null)}
//                     onSuccess={() => {
//                         loadData();
//                     }}
//                 />
...
//                     onSuccess={handleLocalDeleteSuccess}
//                 />
//             )}
...
//                 <StatusModal 
//                     status={globalStatus} 
//                     onClose={() => setGlobalStatus({ type: null, message: null })} 
//                 />
//             )}
//         </div>
//     );
...
const ModifyCollectionModal = ({ collection, onClose, onSuccess }) => {
    ...
    const fetchSemConfig = async () => {
        ...
        try {
            ...
            const response = rawJson.data ? decryptData(rawJson.data) : rawJson;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // PUT Request: Decrypt Response
            const result = rawResult.data ? decryptData(rawResult.data) : rawResult;
    ...
    const executeAction = async () => {
        ...
        try {
            ...
            if (confirmAction === 'upload') {
                ...
                // POST Request (File Upload): FormData cannot be JSON encrypted directly in the same way.
                // Assuming file uploads are standard multipart/form-data and not encrypted JSON body.
                // Standard fetch for file upload.
                const response = await fetch(`${API_URL}/api/admin/addstudents/upload`, {
                    method: 'POST',
                    body: formData,
                    credentials: "include"
                ...
                // DECRYPT RESPONSE (if applicable, file upload responses might be encrypted too)
                const data = rawData.data ? decryptData(rawData.data) : rawData;
...
const DeleteConfirmationModal = ({ collectionName, onClose, onSuccess }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // DELETE Request: Encrypt Payload
            const encryptedBody = encryptData({ collectionNames: [collectionName] });
            ...
            const rawJson = await res.json();
            // DELETE Request: Decrypt Response
