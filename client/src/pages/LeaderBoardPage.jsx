...
const GlassSkeletonLoader = () => (
    <div className="animate-pulse w-full max-w-7xl mx-auto">
        ...
        <div className="hidden md:block mb-12">
            <div className="flex items-end justify-center text-center max-w-5xl mx-auto h-72 gap-1">
...
const LeaderBoardPage = () => {
    ...
    const handlePageChange = (newPage) => {
        ...
        // Scroll to the top of the rankings list smoothly
    ...
    const PlatformScoreLink = ({ score, handle, platformName, baseColor, hoverColor }) => (
        <div className="flex flex-col items-center min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
                <span className={`font-bold text-base ${baseColor} truncate`}>{score || 0}</span>
                {handle && (
    ...
    const renderPodium = () => (
        <div className="hidden md:block mb-12 group">
            <div className="flex items-end justify-center text-center max-w-5xl mx-auto h-72 transform-gpu transition-all duration-500 group-hover:scale-105 gap-1">
                <div className="relative w-1/3 h-[85%] pt-8 pb-4 px-3 bg-slate-800/50 border-2 border-slate-600 rounded-t-xl shadow-lg">
                    ...
                    <div className="text-2xl font-bold text-slate-200 my-2">{topThree[1]?.totalScore || 0}</div>
                ...
                <div className="relative w-1/3 h-full pt-10 pb-5 px-3 bg-yellow-900/30 border-2 border-yellow-500 rounded-t-2xl shadow-2xl shadow-yellow-500/20 z-10">
                    <span className="absolute top-2 right-3 text-5xl font-bold text-yellow-600/30">1</span>
                    <div className="relative w-12 h-12 mx-auto mb-2"><div className="absolute inset-0 bg-yellow-400 rounded-full animate-ping opacity-50"></div><Crown className="relative w-12 h-12 text-yellow-300" /></div>
                    <h3 className="text-base font-bold text-white truncate" title={topThree[0]?.name}>{topThree[0]?.name || 'N/A'}</h3>
                    <div className="text-3xl font-bold text-yellow-100 my-2">{topThree[0]?.totalScore || 0}</div>
                ...
                <div className="relative w-1/3 h-[75%] pt-8 pb-4 px-3 bg-orange-900/40 border-2 border-orange-700 rounded-t-xl shadow-lg">
                    ...
                    <div className="text-2xl font-bold text-orange-200 my-2">{topThree[2]?.totalScore || 0}</div>
                ...
                <main className="px-4 sm:px-6 lg:px-8 py-8">
                    <div className="max-w-7xl mx-auto pb-24">
                        ...
                        {loading ? <GlassSkeletonLoader /> : error ? <div>Error: {error}</div> : (
                            <>
                                {renderPodium()}
                                {/* Set z-index on the filter container to ensure dropdown appears above the list */}
                                ...
                                <div ref={rankingsRef}>
                                    <div className="flex items-center gap-3 mb-4 mt-8"><ListOrdered className="w-6 h-6 text-blue-300" /><h2 className="text-2xl font-semibold text-white">All Rankings</h2></div>
                                                ...
                                                {paginatedCandidates.map(coder => (
                                                    <div key={coder.rollno} className={`transition-all duration-300 hover:bg-white/10 ${coder.rollno === currentUserIdentifier ? 'bg-blue-500/20' : ''}`}>
                                                        ...
                                                        <div className="hidden md:grid grid-cols-12 gap-4 items-center p-4">
                                                            <div className={`col-span-1 font-bold text-lg ${getRankColor(coder.rank)}`}>#{coder.rank}</div>
                                                            <div className="col-span-5"><div className="font-semibold">{coder.name}</div></div>
                                                            <div className="col-span-1 flex items-center justify-center gap-2 text-gray-200">{coder.scores.leetcode} {coder.handles?.leetcode && <a href={coder.handles.leetcode} target="_blank" rel="noopener noreferrer"><ExternalLink size={14} className="opacity-50 hover:opacity-100" /></a>}</div>
                                                            <div className="col-span-1 flex items-center justify-center gap-2 text-gray-200">{coder.scores.gfg} {coder.handles?.gfg && <a href={coder.handles.gfg} target="_blank" rel="noopener noreferrer"><ExternalLink size={14} className="opacity-50 hover:opacity-100" /></a>}</div>
                                                            <div className="col-span-1 flex items-center justify-center gap-2 text-gray-200">{coder.scores.codechef} {coder.handles?.codechef && <a href={coder.handles.codechef} target="_blank" rel="noopener noreferrer"><ExternalLink size={14} className="opacity-50 hover:opacity-100" /></a>}</div>
