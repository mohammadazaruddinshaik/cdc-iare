 ...
 * @date 04 Sep 2025
 * @description An enhanced, role-aware leaderboard with a detailed podium and 50 items per page.
 * @version 5.1.0
 */
...
const LeaderBoardPage = () => {
    ...
    const itemsPerPage = 50; // Set to 50 items per page
    ...
    useEffect(() => {
        const fetchLeaderboardData = async () => {
            try {
                ...
                const response = await fetch(`${BASE_URL}${endpoint}`, { method: "GET", credentials: "include" });
    ...
    // Helper for scores in the main list
    ...
    // Helper for scores in the podium
          ...
          <div className="flex items-end justify-center text-center max-w-5xl mx-auto h-64 lg:h-72 transform-gpu transition-all duration-500 group-hover:scale-105 gap-1">
              ...
              <div className="relative w-1/3 h-[85%] pt-6 pb-4 px-3 bg-slate-800/50 border-2 border-slate-600 rounded-t-xl shadow-lg transition-shadow duration-300 group-hover:shadow-slate-500/20">
                  ...
                  <div className="flex gap-1 pt-2 border-t border-slate-700 px-1 mt-auto">
                      <PodiumPlatformScore score={topThree[1]?.scores?.leetcode} handle={topThree[1]?.handles?.leetcode} platformName="LC" baseColor="text-slate-300" hoverColor="hover:text-white" />
                      ...
                      <PodiumPlatformScore score={topThree[1]?.scores?.codechef} handle={topThree[1]?.handles?.codechef} platformName="CC" baseColor="text-slate-300" hoverColor="hover:text-white" />
                  </div>
              </div>
              {/* 1st Place */}
              <div className="relative w-1/3 h-full pt-8 pb-5 px-3 bg-yellow-900/30 border-2 border-yellow-500 rounded-t-2xl shadow-2xl shadow-yellow-500/20 z-10 transition-shadow duration-300 group-hover:shadow-yellow-400/30">
                  ...
                  <div className="flex gap-1 pt-3 border-t border-yellow-600/50 px-1 mt-auto">
                      <PodiumPlatformScore score={topThree[0]?.scores?.leetcode} handle={topThree[0]?.handles?.leetcode} platformName="LC" baseColor="text-yellow-200" hoverColor="hover:text-white" />
                      ...
                      <PodiumPlatformScore score={topThree[0]?.scores?.codechef} handle={topThree[0]?.handles?.codechef} platformName="CC" baseColor="text-yellow-200" hoverColor="hover:text-white" />
                  </div>
              </div>
              {/* 3rd Place */}
              <div className="relative w-1/3 h-[75%] pt-6 pb-4 px-3 bg-orange-900/40 border-2 border-orange-700 rounded-t-xl shadow-lg transition-shadow duration-300 group-hover:shadow-orange-500/20">
                  ...
                  <div className="flex gap-1 pt-2 border-t border-orange-800 px-1 mt-auto">
                      <PodiumPlatformScore score={topThree[2]?.scores?.leetcode} handle={topThree[2]?.handles?.leetcode} platformName="LC" baseColor="text-orange-300" hoverColor="hover:text-white" />
                      ...
                      <PodiumPlatformScore score={topThree[2]?.scores?.codechef} handle={topThree[2]?.handles?.codechef} platformName="CC" baseColor="text-orange-300" hoverColor="hover:text-white" />
                  </div>
              </div>
          </div>
      </div>
            ...
            <div className="min-h-screen bg-gradient-to-br from-[#071225] via-[#0A1B3A] to-[#071225] text-white">
                ...
                <main className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
                    <div className="max-w-7xl mx-auto pb-24">
                        ...
                        <h1 className="text-3xl lg:text-4xl font-bold flex items-center bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent mb-8">
                                    ...
                                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-4 mt-8">
                                        ...
                                        <div className="flex items-center gap-2 w-full md:w-auto">
                                            ...
                                            <div className="relative w-full md:w-48" ref={batchDropdownRef}>
                                                ...
                                                {isBatchDropdownOpen && (
                                                    <ul className="absolute top-full right-0 mt-2 w-full bg-[#0A1B3A]/95 backdrop-blur-lg border border-white/10 rounded-lg shadow-lg max-h-60 overflow-y-auto z-20 custom-scrollbar">
                    ...
                    {userRole === 'student' && currentUserData && !loading && (
                        <div className={`fixed bottom-4 left-4 right-4 z-50 transition-all duration-500 ${animate ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
                            <div className="max-w-7xl mx-auto"><div className="bg-gradient-to-r from-blue-500/30 to-purple-500/30 backdrop-blur-xl border border-white/20 rounded-2xl p-4 shadow-2xl flex items-center justify-between gap-4"><div className="flex items-center gap-4"><div className={`font-bold text-2xl ${getRankColor(currentUserData.rank)}`}>#{currentUserData.rank}</div><div><div className="font-semibold text-white text-lg">{currentUserData.name}</div><div className="text-sm text-blue-200">Your Current Position</div></div></div><div className="text-right"><div className="font-bold text-2xl text-white">{currentUserData.totalScore}</div><div className="text-sm text-blue-200">Total Score</div></div></div></div>
                        </div>
                    )}
                </main>
            </div>
