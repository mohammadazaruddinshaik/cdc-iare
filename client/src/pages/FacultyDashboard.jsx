...
//         Saturday: [{ time: "1:15PM - 3:50PM", subject: "JFS", room: "5106", type: "Lecture" }]
//     }
// };
...
const getCourseIcon = (subject) => {
    ...
    if (subject.includes("DBS")) return <Database className="w-5 h-5 text-green-400" />;
...
const FacultyDashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                sessionStorage.setItem("facultybatches", JSON.stringify(data.faculty.batches_assigned));
                ...
                sessionStorage.clear(); // Clear all session data on error
    ...
    // The error display is no longer needed as the component will redirect
    if (!dashboardData) {
        return null; 
    }
                                ...
                                <div key={coder.rollno} className={`${getRankBadge(coder.rank)} relative overflow-hidden rounded-2xl p-4 flex flex-col justify-between transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 hover:scale-105 min-h-[160px]`} style={{ animationDelay: `${index * 100}ms` }}>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-1 text-xs text-gray-700 mb-2">
                                            <div className="bg-white/50 backdrop-blur-sm px-2 py-0.5 rounded-full shadow-md"><span className="font-semibold">LeetCode:</span> {coder.scores.leetcode}</div>
                                            <div className="bg-white/50 backdrop-blur-sm px-2 py-0.5 rounded-full shadow-md"><span className="font-semibold">GeeksforGeeks:</span> {coder.scores.gfg}</div>
                                            <div className="bg-white/50 backdrop-blur-sm px-2 py-0.5 rounded-full shadow-md"><span className="font-semibold">CodeChef:</span> {coder.scores.codechef}</div>
                                        </div>
                                    </div>
